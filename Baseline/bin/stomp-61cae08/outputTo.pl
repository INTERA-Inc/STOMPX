#!/usr/bin/perl
# outputTo.pl 1320 2020-03-08 23:39:35Z d3e144 https://stash.pnnl.gov/scm/stomp/stomp.git V3.0
#
#  Print banner.
#
print("\nWelcome to outputTo ...\n\n");
print("This perl program transforms a STOMP output file into\n");
print("formatted input file for Gnuplot, Grapher, Igor, MatLab, Plotmtv, Tecplot, CSV, and SciDAVis.\n");
...
while ( $ARGV[0] =~ /^-/i ) {
  if( $ARGV[0] =~ /help\b/i ) {
    ...
    print "    [Options]\n";
    print "    Plotting Package Option [Gnuplot|Grapher|Igor|MatLab|Plotmtv|Tecplot|CSV|SciDAVis]\n";
...
#
  ...
  $stops = 0;
  do {
    print("Plotting Package [Gnuplot|Grapher|Igor|Matlab|Plotmtv|Tecplot|CSV|SciDAVis]?\n");
    $plot_package = <STDIN>;
    chomp( $plot_package );
    if( $plot_package =~ /^tecplot\b/i ) {
      print("Plotting Package: Tecplot\n");
...
#
#   Read the reference-triangle indices
#
  if( $tflag == 1 ) {
    @fields = split(/\s+/,$output_line);
...
#
#   Read the reference-borehole node indices
#
  if( $bflag == 1 ) {
    @fields = split(/\s+/,$output_line);
...
#
  if( $plot_package =~ /^igor\b/i ) {
    if( $rflag == 1 && $aflag == 0) {
      ...
      if( $fields[0] =~ /Step\b/ ) {
        $aflag = 1;
        for( $j = $jstart; $j <= $#fields; $j++ ) {
          push( @abbrev,$fields[$j] );
        }
#
#       Write reference-node variable abbreviations with node index.
#       For species and solute variables write specie or solute name 
#       in addition to abbreviation
#
        print OUT "TM";
        for( $j = 0; $j <= $#nn; $j++ ) {
          for( $i = 0; $i <= $#v_list; $i++ ) {
            if (index($v_unit[$i] , "Species") != -1) {
              $left = index($v_unit[$i], '  ');
              $right = index($v_unit[$i], 'Species');
              $length = $right - $left - 3;
              $specie =  substr $v_unit[$i], $left+2, $length;
#              print OUT " ${specie}_$nn[$j]";
              print OUT " $abbrev[$v_list[$i]]_${specie}_$nn[$j]";
            } elsif (index($v_unit[$i] , "Solute") != -1) {
              $left = index($v_unit[$i], '  ');
              $right = index($v_unit[$i], 'Solute');
              $length = $right - $left - 3;
              $specie =  substr $v_unit[$i], $left+2, $length;
#              print OUT " ${specie}_$nn[$j]";
              print OUT " $abbrev[$v_list[$i]]_${specie}_$nn[$j]";
...
#
        if ( $nr == 0 ) {
          ...
          if ($av_opt == 1) {
            ...
            for( $i = 0; $i <= $#v_list; $i++ ) {
              if ($geomech==1) {
                push( @record,$fields[$v_list[$i]+$jstart] );
              } else {
...
#
#  Start reading reference-triangle indices on the next line
#
  if( $output_line =~ /Fracture Nos., Local Triangle Nos./ ) {
    $tflag = 1;
  }
#
#  Start reading reference-borehole node indices on the next line
#
  if( $output_line =~ /Borehole Nos., Local Borehole Nos./ ) {
    $bflag = 1;
  }
#
...
#      print OUT ("% subtitle = \"Reference Node ($in[$j],$jn[$j],$kn[$j]: $nn[$j])\"\n");
      ...
      if( $v_unit[$i] ) {
        if (index($v_unit[$i] , "Species") != -1) {
#          $left = index($v_unit[$i], '  ');
          $length = index($v_unit[$i], 'Species');
#          $length = $right - $left - 3;
          $specie =  substr $v_unit[$i], $left+2, $length;
#          print OUT ",${specie}_$nn[$j]";
        print OUT ("% ylabel = \"$v_name[$i], ${specie}_$nn[$j]\"\n");
        }
      }   else {
...
if( $plot_package =~ /^gnuplot\b/i ) {
  for( $i = 0; $i <= $#v_name; $i++ ) {
    if (index($v_unit[$i] , "Species") != -1) {
      ...
      $length = $right - $left - 3;
      $specie =  substr $v_unit[$i], $left+2, $length;
      $specie_unit = substr $v_unit[$i], 0, $right;
      $gnu_file = ${specie} . ".gnu";
      $png_file = ${specie} . ".png";
    }  else  {
      $gnu_file = $abbrev[$i] . ".gnu";
      $png_file = $abbrev[$i] . ".png";
    }
    open( GNU,">$gnu_file") || die "Error: Unable to Open Gnuplot Script File: $gnu_file.\n";
    print GNU "set terminal png font \"Times,16\"\n";
    ...
    if( $v_unit[$i] ) {
      print GNU "set ylabel \"$v_name[$i], ${specie_unit}\"\n";
    } else {
