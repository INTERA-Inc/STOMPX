#!/usr/bin/perl
# plotTo.pl 1269 2018-12-31 15:38:48Z d3c002 https://stash.pnnl.gov/scm/stomp/stomp.git V3.0
print("\nWelcome to plotTo ...\n\n");
print("This perl program transforms STOMP plot file(s) into\n");
print("formatted input files for Gnuplot, Matlab, Plotmtv, Surfer, Tecplot, VTK, SCiDAVis, and OpenIAM.\n");
print("VTK format can be read by VisIt and Paraview.\n");
print("Multiple plot files are allowable for Gnuplot, Plotmtv, Tecplot and OpenIAM;\n");
...
while ( $ARGV[0] =~ /^-/i ) {
  if( $ARGV[0] =~ /help\b/i ) {
    ...
    print "    Plotting Package Option [Gnuplot|MatLab|Plotmtv|Surfer|Tecplot|TecplotXY|Tecplot10|VTK|CSV|OpenIAM]\n";
    ...
    print "-ts        use time stamp for Gnuplot or Tecplot zone names. For csv files time stamp is appended to header titles\n";
    ...
    print "-layer     Extract one z plane (model layer) (OpenIAM only)\n\n";
    ...
    print "plotTo.pl OpenIAM plots.dat plot.*\n";
    print "plotTo.pl -ts csv plot_123.csv plot.123\n";
...
#
    ...
    shift(@ARGV);
#
#  OpenIAM single layer
#
  } elsif( $ARGV[0] =~ /\-layer\b/ ) {
    $layer_opt = 1;
    shift(@ARGV);
#
#  Unrecognized option
...
#  chomp( $plot_package );
  ...
  } elsif( $plot_package =~ /^csv\b/i ) {
    print("Plotting Package: CSV\n");
  } elsif( $plot_package =~ /^openiam\b/i ) {
    print("Plotting Package: OpenIAM\n");
 } else {
...
#
    if( $ARGV[2] ) {
      print( "Plot File(s): \n" );
#
#  Check for multiple plot files with the Matlab, Surfer, VTK or CSV (Excel) plotting package
#
      ...
      if( $plot_package =~ /^csv\b/i && $#ARGV > 2 ) {
        die "Error: Multiple Plot Files Specified for the CSV Option.\n";
      }
      for( $nf = 2; $nf <= $#ARGV; $nf++ ) {
...
#
      ...
      do {
        ...
        @plot_file = split(/,+/,$names);
#
#  Check for multiple plot files with the Matlab, Surfer, VTK or CSV plotting packages
#
        ...
        if( $plot_package =~ /^csv\b/i && $#plot_file > 0 ) {
          die "Error: Multiple Plot Files Specified for the CSV Option.\n";
        }
...
#
    ...
    do {
      if( $plot_package =~ /^tecplot\b/i ) {
        ...
        print("CSV File to Generate?\n");
       } elsif( $plot_package =~ /^openiam\b/i ) {
        print("OpenIAM File prefix to Generate?\n");
     }
      $out_file = <STDIN>;
      chomp( $out_file );
...
#
    do {
      ...
      @plot_file = split(/,+/,$names);
#
#  Check for multiple plot files with the Matlab, Surfer, VTK or CSV plotting package
#
      if( $plot_package =~ /^matlab\b/i && $#plot_file > 0 ) {
        die "Error: Multiple Plot Files Specified for the Matlab Plotting Package.\n";
      }
...
#
  ...
  $stops = 0;
  do {
    print("Plotting Package [Gnuplot|MatLab|Plotmtv|Surfer|Tecplot|TecplotXY|Tecplot10|VTK|CSV|OpenIAM]?\n");
    $plot_package = <STDIN>;
    chomp( $plot_package );
    if( $plot_package =~ /^tecplot\b/i ) {
      print("Plotting Package: Tecplot\n");
...
#
  ...
  do {
      if( $plot_package =~ /^tecplot\b/i ) {
        ...
        print("OpenIAM File to Generate?\n");
     }
    $out_file = <STDIN>;
    chomp( $out_file );
    if( open( OUT,">$out_file") ) {
...
#
  if( ($plot_package =~ /^tecplot\b/i || $plot_package =~ /^gnuplot\b/i || 
    ...
    $plot_package =~ /^csv\b/i  || $plot_package =~ /^tecplotxy\b/i)
    && $ts_opt == 1 ) {
    $stops = 0;
    do {
      print("Time Stamp Unit?\n");
...
#
#  Set time and unit conversion flags for OpenIAM
#
  if( $plot_package =~ /^openiam\b/i ) {
        $ts_opt = 1;
        $tsunit = "yr";
        $lconvert = 1.0;
        $pconvert = 1.0;
        $psiconvert = 6.8948e+3;
        $mpaconvert = 1.e+6;
        $ftconvert = 3.048e-1;
  }
#
...
#
      ...
      $nbrn = $fields[$#fields];
#
#  Set the number of leaky well nodes
#
    } elsif( $plot_line =~ /Number of Leaky Well Nodes/ ) {
      @fields = split(/\s+/,$plot_line);
      $nlwn = $fields[$#fields];
#
...
#
  if( $plot_package =~ /^csv\b/i ) {
#
#  Shorten header titles by assigning abbreviations
#
    for( $i = 0; $i <= $#fvname; $i++ ) {
      $fvname[$i] =~ s/Aqueous/Aq/g;
      $fvname[$i] =~ s/Saturation/Sat/g;
      $fvname[$i] =~ s/Direction/Dir/g;
      $fvname[$i] =~ s/Temperature/Temp/g;
      $fvname[$i] =~ s/Permeability/Perm/g;
      $fvname[$i] =~ s/Porosity/Por/g;
      $fvname[$i] =~ s/Intrinsic/Intr/g;
      $fvname[$i] =~ s/Diffusive/Diff/g;
      $fvname[$i] =~ s/Pressure/Pres/g;
      $fvname[$i] =~ s/Displacement/Disp/g;
      $fvname[$i] =~ s/Diffusive/Diff/g;
      $fvname[$i] =~ s/Effective/Eff/g;
      $fvname[$i] =~ s/Trapped/Tpd/g;
      $fvname[$i] =~ s/Integrated/Int/g;
      $fvname[$i] =~ s/Vertically/Vert/g;
      $fvname[$i] =~ s/Fraction/Frac/g;
      $fvname[$i] =~ s/Velocity/Vel/g;
      $fvname[$i] =~ s/Centered/Cntrd/g;
      $fvname[$i] =~ s/Position/Pos/g;
    }
#
#  Add time stamp to all header titles and print
#  
#    $tsdec = 0+"$tsv[$nf]";
#    $tsstr = substr($tsdec,0,5);
#    $tsvu = join ("", substr($tsv[$nf],0,6), $tsunit);
    $tsvu = join ("", substr($tsv[$nf],0,4),substr($tsv[$nf],8,11), $tsunit);
#    $tsvu = join ("", $tsv[$nf], $tsunit);
#    print "$nf,$tsv[$nf],$tsunit,$tsdec,$tsvu\n";
#  X-direction vertices only
#
    if( $xvert == 1 && $yvert != 1  && $zvert != 1 && $ifld > 1 ) {
      print OUT "X($xunit)_$tsvu,Vol($nvunit)_$tsvu";
      for( $i = 0; $i <= $#fvname; $i++ ) {
        if( $fvunit[$i] ) {
          print OUT ",$fvname[$i]($fvunit[$i])_$tsvu";
        } else {
          print OUT ",$fvname[$i]_$tsvu";
        }
      }
      print OUT "\n";
      for( $i = 0; $i < $nfld; $i++ ) {
...
#
      ...
      print OUT "Y($yunit)_$tsvu,Vol($nvunit)_$tsvu";
      for( $i = 0; $i <= $#fvname; $i++ ) {
        if( $fvunit[$i] ) {
          print OUT ",$fvname[$i]($fvunit[$i])_$tsvu";
        } else {
...
#
      ...
      print OUT "Z($zunit)_$tsvu,Vol($nvunit)_$tsvu";
      for( $i = 0; $i <= $#fvname; $i++ ) {
        if( $fvunit[$i] ) {
          print OUT ",$fvname[$i]($fvunit[$i])_$tsvu";
        } else {
...
#
      ...
      print OUT "X($xunit)_$tsvu,Y($yunit)_$tsvu,Vol($nvunit)_$tsvu";
      for( $i = 0; $i <= $#fvname; $i++ ) {
        if( $fvunit[$i] ) {
          print OUT ",$fvname[$i]($fvunit[$i])_$tsvu";
        } else {
...
#
      ...
      print OUT "X($xunit)_$tsvu,Z($zunit)_$tsvu,Vol($nvunit)_$tsvu";
      for( $i = 0; $i <= $#fvname; $i++ ) {
        if( $fvunit[$i] ) {
          print OUT ",$fvname[$i]($fvunit[$i])_$tsvu";
        } else {
...
#
      ...
      print OUT "Y($yunit)_$tsvu,Z($zunit)_$tsvu,Vol($nvunit)_$tsvu";
      for( $i = 0; $i <= $#fvname; $i++ ) {
        if( $fvunit[$i] ) {
          print OUT ",$fvname[$i]($fvunit[$i])_$tsvu";
        } else {
...
#
      ...
      print OUT "X($xunit)_$tsvu,Y($yunit)_$tsvu,Z($zunit)_$tsvu,Vol($nvunit)_$tsvu";
      for( $i = 0; $i <= $#fvname; $i++ ) {
        if( $fvunit[$i] ) {
          print OUT ",$fvname[$i]($fvunit[$i])_$tsvu";
        } else {
...
#
#  Store data for OpenIAM output file
#  
  if( $plot_package =~ /^openiam\b/i ) {
#
#  First file only 
#
    if( $nf == 0 ) {
#
#  Request layer to extract
#
      $stops = 0;
      do {
        print("Layer to extract (k index)\n");
        $layer = <STDIN>;
        chomp( $layer );
        if( $layer > $kfld ) {
          print("Error: Layer number must be less than or equal to $kfld. Try again!\n\n");
        } else {
          $stops = 1;
        }
      } until $stops != 0;
#
#  convert length units to meters if needed
#
      if( $xunit =~ /ft\b/i ) {
        $lconvert = $ftconvert;
      }
...
#    
#  Now for all time files
#
#  Put gas pressure field variable for all layers into array
#  Convert units if needed
#
    for( $ii = 0; $ii <= $#fvname; $ii++ ) {
      if( $fvname[$ii] =~ /gas\ pressure\b/i ) {
        print "$fvunit[$ii]\n"; 
        $pv = $ii;
        $j = $ii;
        if( $fvunit[$ii] =~ /psi\b/i ) {
          $pconvert = $psiconvert;
        } elsif( $fvunit[$ii] =~ /mpa\b/i ) {
            $pconvert = $mpaconvert;
        }  
        print "$pconvert\n";   
        for( $n = 0; $n < $nfld; $n++ ) {
...
#
#  Put gas saturation field variable for all layers into array
#
    for( $ii = 0; $ii <= $#fvname; $ii++ ) {
      if( $fvname[$ii] =~ /gas\ saturation\b/i ) {
        $sv = $ii;
        $j = $ii;
        for( $n = 0; $n < $nfld; $n++ ) {
          $k = $j*$nfld + $n;
...
# end of plot file loop
#
#  Write OpenIAM outputs
#
#  Write time series file
#
if( $plot_package =~ /^openiam\b/i ) {
  $out_file2 = "time_points.csv";
  open( OUTTIM,">$out_file2") || die "Error: Unable to Create Time Points File: $out_file2\n";
  local $, = ',';
  print OUTTIM @tsv;
  close( OUTTIM );
#
#  Write OpenIAM reservoir lookup table file in csv format containing 
#. gas pressure and gas saturation filed variables
#
  print OUT "#X(m),Y(m)";
  for( $nt = 1; $nt <= $ntim; $nt++ ) {
    print OUT ",pressure$nt";
  }
  for( $nt = 1; $nt <= $ntim; $nt++ ) {
      print OUT ",saturation$nt";
  }
  print OUT "\n";
# 
# Only write out pressure and saturation for selected layer
#  
#  print scalar @pres_arr;
#  print "\n";
#  print scalar @sat_arr;
#  print "\n";
#  print scalar @fv;
#  print "\n";
#  print "\n";
#  print "$nfld,$ntim\n";
  $ijfld = $ifld * $jfld;
  for( $i = 0; $i < $ijfld; $i++ ) {
    print OUT "$x_arr[$i],$y_arr[$i]";
#    print "$nfld,$pv,$ijfld,$layer,$i\n";
#    $indexp = ($nfld*$pv) + ijfld * ($layer-1) + $i;
#    $indexs = ($nfld*$sv) + ijfld * ($layer-1) + $i;
#    $index = $nfld + (ijfld * ($layer-1)) + $i;
#    $indexs = ($nfld*$sv) + ijfld * ($layer-1) + $i;
#    print "$index\n";
    for( $nt = 1; $nt <= $ntim; $nt++ ) {
#      $idx = ($nt+1)*$i;
#      $index = $nfld + (ijfld * ($layer-1)) + $i;
#      $idx = ($nfld*$nt) + $ijfld * ($layer-1) + $i;
      $idx = $nt *(($ijfld * ($layer-1)) + $i);
      print OUT ",$pres_arr[$idx]";
      print "pres,$idx,$pres_arr[$idx]\n";
    }
    for( $nt = 1; $nt <= $ntim; $nt++ ) {
      $idx = $nt *(($ijfld * ($layer-1)) + $i);
      print OUT ",$sat_arr[$idx]";
      print "sat,$idx,$sat_arr[$idx]\n";
    }
    print OUT "\n";
#    exit;
  }
