...
!        IF( N.EQ.11 ) PRINT *,'PG(2,',ND(N),') = ',PG(2,N),' ID = ',ID
      ENDDO
!
!---  Read local copies of water vapor pressure 
!     (including ghost cells)  ---
!
      OFFSET = IOFFSET + NC*NBYTR
      IOFFSET = IOFFSET + NFCGC_G*NBYTR
     ...
     &  STATUS,IERR)
      DO N = 1,NFCGC(ID+1)
        PVW(2,N) = VARX(N)
!        IF( N.EQ.11 ) PRINT *,'PVW(2,',ND(N),') = ',PVW(2,N),' ID = ',ID
      ENDDO
!
...
!     &    ' ID = ',ID
      ENDDO
!
!---  Read local copies of total gas saturation
!     (including ghost cells)  ---
!
      OFFSET = IOFFSET + NC*NBYTR
      IOFFSET = IOFFSET + NFCGC_G*NBYTR
...
!        IF( N.EQ.11 ) PRINT *,'SG(2,',ND(N),') = ',SG(2,N),' ID = ',ID
      ENDDO
!
!---  Read local copies of CO2 aqueous mass fraction
!     (including ghost cells)  ---
!
      OFFSET = IOFFSET + NC*NBYTR
      IOFFSET = IOFFSET + NFCGC_G*NBYTR
     ...
     &  STATUS,IERR)
      DO N = 1,NFCGC(ID+1)
        XLA(2,N) = VARX(N)
!        IF( N.EQ.11 ) PRINT *,'XLA(2,',ND(N),') = ',XLA(2,N),' ID = ',ID
      ENDDO
!
...
!        IF( N.EQ.11 ) PRINT *,'YLS(2,',ND(N),') = ',YLS(2,N),' ID = ',ID
      ENDDO
!
!---  Read local copies of total salt mass
!     (including ghost cells)  ---
!
      OFFSET = IOFFSET + NC*NBYTR
      IOFFSET = IOFFSET + NFCGC_G*NBYTR
     ...
     &  STATUS,IERR)
      DO N = 1,NFCGC(ID+1)
        TMS(2,N) = VARX(N)
!        IF( N.EQ.11 ) PRINT *,'TMS(2,',ND(N),') = ',TMS(2,N),' ID = ',ID
      ENDDO
!
...
!
     ...
     &    STATUS,IERR )
!
!---  Load temporary array with water vapor pressure  ---
!
      DO N = 1,NFCGC(ID+1)
        VARX(N) = PVW(2,N)
      ENDDO
!
!---  Write water vapor pressure to restart file  ---
!
      OFFSET = IOFFSET + NC*NBYTR
      IOFFSET = IOFFSET + NFCGC_G*NBYTR
      CALL MPI_FILE_WRITE_AT( IRS,OFFSET,VARX,NFCGC(ID+1),MPI_REAL8,
...
!
     ...
     &    STATUS,IERR )
!
!---  Load temporary array with total gas saturation  ---
!
      DO N = 1,NFCGC(ID+1)
        VARX(N) = SG(2,N)
      ENDDO
!
!---  Write total gas saturation to restart file  ---
!
      OFFSET = IOFFSET + NC*NBYTR
      IOFFSET = IOFFSET + NFCGC_G*NBYTR
      CALL MPI_FILE_WRITE_AT( IRS,OFFSET,VARX,NFCGC(ID+1),MPI_REAL8,
     &    STATUS,IERR )
!
!---  Load temporary array with CO2 aqueous mass fraction  ---
!
      DO N = 1,NFCGC(ID+1)
        VARX(N) = XLA(2,N)
      ENDDO
!
!---  Write CO2 aqueous mass fraction to restart file  ---
!
      OFFSET = IOFFSET + NC*NBYTR
      IOFFSET = IOFFSET + NFCGC_G*NBYTR
      CALL MPI_FILE_WRITE_AT( IRS,OFFSET,VARX,NFCGC(ID+1),MPI_REAL8,
...
!
     ...
     &    STATUS,IERR )
!
!---  Load temporary array with total salt mass  ---
!
      DO N = 1,NFCGC(ID+1)
        VARX(N) = TMS(2,N)
      ENDDO
!
!---  Write total salt mass to restart file  ---
!
      OFFSET = IOFFSET + NC*NBYTR
      IOFFSET = IOFFSET + NFCGC_G*NBYTR
      CALL MPI_FILE_WRITE_AT( IRS,OFFSET,VARX,NFCGC(ID+1),MPI_REAL8,
