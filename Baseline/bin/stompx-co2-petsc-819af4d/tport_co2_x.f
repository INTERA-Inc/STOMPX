...
!
      ...
      INDX = 1
      CALL STOMP_PETSC_SOLVE( T_KSP,T_MAT,T_RHS_VEC,T_SOL_VEC,
...
!
      ...
      DO NEQ = 1,NUKTL(ID+1)
        ...
        DO NCOL = NLUC(NEQ),(NLUC(NEQ+1)-1)
          ...
          ICOL(MC) = MLUC(NC)
        ENDDO
        IROW = NEQ + NUKTO(ID+1) - 1
        CALL MatSetValues( T_MAT,1,IROW,MC,ICOL,BUFFER,
     &    INSERT_VALUES,IERR )
      ENDDO
...
!
          ...
          BUFFER = AB*BCX(1)
          IROW_P = MP - 1
          CALL VecSetValues( T_RHS_VEC,1,IROW_P,BUFFER,ADD_VALUES,IERR ) 
