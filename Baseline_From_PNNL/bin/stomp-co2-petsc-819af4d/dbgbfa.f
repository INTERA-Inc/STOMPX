...
!     dbgbfa.F https://stash.pnnl.gov/scm/stomp/stomp.git V3.0
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!---  Function SAXPY  ---
!
               IF( LM.EQ.0 .OR. ABS(TT).LE.1.D-37 ) GOTO 78
               DO 75 I = 1,LM
                 ALU(MM+I,J) = TT*ALU(M+I,K) + ALU(MM+I,J)
                 IF( ABS(ALU(MM+I,J)).LE.1.D-37 ) ALU(MM+I,J) = 0.D+0
   75          CONTINUE
   78          CONTINUE
   80       CONTINUE
   90       CONTINUE
