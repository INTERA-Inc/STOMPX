...
!
        IF( K.NE.1 .AND. INBS(1,N).EQ.0 ) THEN
          NB = ICM(1,1,N)
          IF( NB.EQ.0 ) GOTO 3210
          IF( IXP(NB).EQ.0 ) GOTO 3210
          NMD = ABS(IXP(NB))
          DO 3200 M = 1,ISVC
            DNRX = DNR(M,NB)
            ICOL(M) = IM(M,NMD)-1
...
!
        IF( J.NE.1 .AND. INBS(2,N).EQ.0 ) THEN
          NS = ICM(1,2,N)
          IF( NS.EQ.0 ) GOTO 3310
          IF( IXP(NS).EQ.0 ) GOTO 3310
          NMD = ABS(IXP(NS))
          DO 3300 M = 1,ISVC
            DNRX = DNR(M,NS)
            ICOL(M) = IM(M,NMD)-1
...
!
        IF( I.NE.1 .AND. INBS(3,N).EQ.0 ) THEN
          NW = ICM(1,3,N)
          IF( NW.EQ.0 ) GOTO 3410
          IF( IXP(NW).EQ.0 ) GOTO 3410
          NMD = ABS(IXP(NW))
          DO 3400 M = 1,ISVC
            DNRX = DNR(M,NW)
            ICOL(M) = IM(M,NMD)-1
...
!
        IF( I.NE.IFLD .AND. INBS(4,N).EQ.0 ) THEN
          NE = ICM(1,4,N)
          IF( NE.EQ.0 ) GOTO 3510
          IF( IXP(NE).EQ.0 ) GOTO 3510
          NMD = IXP(NE)
          DO 3500 M = 1,ISVC
            DNRX = DNR(M,NE)
            ICOL(M) = IM(M,NMD)-1
...
!
        IF( J.NE.JFLD .AND. INBS(5,N).EQ.0 ) THEN
          NN = ICM(1,5,N)
          IF( NN.EQ.0 ) GOTO 3610
          IF( IXP(NN).EQ.0 ) GOTO 3610
          NMD = ABS(IXP(NN))
          DO 3600 M = 1,ISVC
            DNRX = DNR(M,NN)
            ICOL(M) = IM(M,NMD)-1
...
!
        IF( K.NE.KFLD .AND. INBS(6,N).EQ.0 ) THEN
          NT = ICM(1,6,N)
          IF( NT.EQ.0 ) GOTO 3710
          IF( IXP(NT).EQ.0 ) GOTO 3710
          NMD = ABS(IXP(NT))
          DO 3700 M = 1,ISVC
            DNRX = DNR(M,NT)
            ICOL(M) = IM(M,NMD)-1
