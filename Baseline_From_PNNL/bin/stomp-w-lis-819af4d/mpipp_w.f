...
!
      CALL WRITE_SOLU_W( IDX,JDX,KDX )
!
!---  Write grid data to a series of binary grid files  ---
!
      CALL WRITE_GRID_W( IDPX,JDPX,KDPX )
!
...
!
      CALL WRITE_COUP_WELL_W( IDX,JDX,KDX )
!
...
!
      IF( ISLC(50).NE.0 ) CALL WRITE_GMEC_W
!
!---  Write geomechanics grid data to gmgd.bin  ---
!
      IF( ISLC(50).NE.0 ) CALL WRITE_GMGD_W
!
!---  Write geomechanics boundary condition data to gmbc.bin  ---
!
      IF( ISLC(50).NE.0 ) CALL WRITE_GMBC_W( IDX,JDX,KDX )
...
!----------------------Type Declarations-------------------------------!
!
      REAL*8, DIMENSION(:), ALLOCATABLE :: PLBX
...
!
!---  Write boundary condition input variables to boco.bin  --
!
      WRITE(20) (((BC(L,M,N),L=1,LBCV),M=1,LBTM),N=1,LBCIN)
      WRITE(20) (TSBC(N),N=1,LBCIN)
      WRITE(20) (XSBC(N),N=1,LBCIN)
      WRITE(20) (YSBC(N),N=1,LBCIN)
      WRITE(20) (ZSBC(N),N=1,LBCIN)
!
...
!----------------------Subroutine--------------------------------------!
!
      SUBROUTINE WRITE_COUP_WELL_W( IDX,JDX,KDX )
!
!-------------------------Disclaimer-----------------------------------!
!
!     This material was prepared as an account of work sponsored by
...
!     STOMP-W
!
!     Write state condition data to well.bin
!
!----------------------Authors-----------------------------------------!
!
!     Written by M.D. White, PNNL, 11 January 2022.
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!
      ISUB_LOG = ISUB_LOG+1
      SUB_LOG(ISUB_LOG) = '/WRITE_COUP_WELL_W'
!
...
!
      ISUB_LOG = ISUB_LOG-1
!
!---  End of WRITE_COUP_WELL_W group
!
      RETURN
      END
...
!----------------------Subroutine--------------------------------------!
!
      SUBROUTINE WRITE_GMBC_W( IDX,JDX,KDX )
!
!-------------------------Disclaimer-----------------------------------!
!
!     This material was prepared as an account of work sponsored by
...
!----------------------Subroutine--------------------------------------!
!
      SUBROUTINE WRITE_GRID_W( IDPX,JDPX,KDPX )
!
!-------------------------Disclaimer-----------------------------------!
!
!     This material was prepared as an account of work sponsored by
...
!----------------------Type Declarations-------------------------------!
!
      REAL*8, DIMENSION(:), ALLOCATABLE :: RHOSX
      REAL*8, DIMENSION(:,:), ALLOCATABLE :: CMPX,PORX,TORX,PERMX
      REAL*8, DIMENSION(:,:), ALLOCATABLE :: SCHRX
      REAL*8, DIMENSION(:,:), ALLOCATABLE :: RPLCX
      INTEGER, DIMENSION(:), ALLOCATABLE :: ITORX,IPRFX,ISMX,ISCHRX
      INTEGER, DIMENSION(:), ALLOCATABLE :: IRPLX,ISKPX
      INTEGER, DIMENSION(:,:), ALLOCATABLE :: ISLTBLX,IRLTBLX
!
!----------------------Executable Lines--------------------------------!
!
      ISUB_LOG = ISUB_LOG+1
      SUB_LOG(ISUB_LOG) = '/WRITE_PROP_W'
!
...
!----------------------Subroutine--------------------------------------!
!
      SUBROUTINE WRITE_SOLU_W( IDX,JDX,KDX )
!
!-------------------------Disclaimer-----------------------------------!
!
!     This material was prepared as an account of work sponsored by
...
!     write the array to the grid.bin file  --
!
...
!
      ...
      ENDIF
!
!---  Write execution period values to the grid.bin file  --
!
...
!
!---  Write unit conversions for outputs to the grid.bin file  ---
!
      ...
      ENDDO
!
!---  Write output unit conversions to the grid.bin file  ---
!
      WRITE(20) CNVTM
      WRITE(20) CNVLN
      ...
      ENDIF
!
!---  Write output unit to the grid.bin file  ---
!
      WRITE(20) UNTM
      WRITE(20) UNLN
...
!
!---  Write output control variables to the grid.bin file  ---
!
      WRITE(20) (PRTM(M),M=1,LPTM)
!
!---  Write output control integer arrays to the grid.bin file  ---
!
...
!     write the surface flux file names to the grid.bin file  ---
!
      DO M = 1,LSF
        NCH = INDEX(FNSF(M)(1:),'  ')
...
!
      ISUB_LOG = ISUB_LOG+1
      SUB_LOG(ISUB_LOG) = '/WRITE_STATE_W'
!
!---  Open a new state.bin file  --
!
      OPEN( UNIT=20,FILE='state.bin',STATUS='UNKNOWN',
     &  FORM='UNFORMATTED' )
      CLOSE(UNIT=20,STATUS='DELETE')
      OPEN(UNIT=20, FILE='state.bin', STATUS='NEW', 
     &  FORM='UNFORMATTED')
!
...
!
      ...
      INTEGER, DIMENSION(:), ALLOCATABLE :: IPCSLX
!
!----------------------Executable Lines--------------------------------!
!
      ISUB_LOG = ISUB_LOG+1
      SUB_LOG(ISUB_LOG) = '/WRITE_TPORT_W'
!
...
!       index  --
!
        ALLOCATE( IPCSLX(1:LFD),STAT=ISTAT )
        IF( ISTAT.NE.0 ) THEN
          INDX = 3
          CHMSG = 'Allocation Error: IPCSLX'
          CALL WRMSGS( INDX )
        ENDIF
!
!---    Write solute solid-aqueous partition index  ---
!
        DO NSL = 1,NSOLU
!
!---      Fill temporary solid-aqueous partition array index  --
!
          DO N = 1,NFLD
            IF( IZ(N).LE.0 ) THEN
              IPCSLX(N) = 0
            ELSE
              IPCSLX(N) = IPCSL(IZ(N),NSL)
            ENDIF
          ENDDO
          WRITE(20) ((IPCSLX(ND_MPI(K,L)),K=1,NC_MPI(L)),L=1,NP_MPI)
        ENDDO
!
!---    Deallocate temporary memory for solid-aqueous partition array
!       index  --
!
        DEALLOCATE( IPCSLX,STAT=ISTAT )
        IF( ISTAT.NE.0 ) THEN
          INDX = 3
