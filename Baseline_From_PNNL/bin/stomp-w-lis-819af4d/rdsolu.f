...
!     ISLC(14):  0    No SPLIB summary output w/ absolute residual
!                1    SPLIB summary output w/ absolute residual
!                10   No SPLIB summary output w/ scaling w/ abs. residual
!                11   SPLIB summary output w/ scaling w/ abs. residual
!                100  No SPLIB summary output w/ relative residual
!                101  SPLIB summary output w/ relative residual
!                110  No SPLIB summary output w/ scaling w/ rel. residual
!                111  SPLIB summary output w/ scaling w/ rel. residual
...
!                3    Isobrine with low-temperature Spycher equilibrium
...
!               31    Fracture/Fault Flow Problem Vector
!               32    Fracture/Fault Flow Matrix
!               33    Fracture/Fault Flow Solution Vector             
!     ISLC(35)   0    Limited Fracture/Fault Output
!                1    Full Fracture/Fault Output
!     ISLC(36)   0    Limited Borehole Output
!                1    Full Borehole Output
!     ISLC(37):  0    Nonisoair
!                1    Isoair
!     ISLC(38):  0    Porous media matrix
!                1    Hot dry rock porous matrix
!     ISLC(39):  0    Default convergence criterion
!                1    First quartile convergence criterion
...
!     ISLC(67):  0    No MPI preprocessing
!                1    MPI preprocessing
...
!     ISLC(74):  0    No Fracture and Borehole Flow
!                1    Fracture Flow
!                2    Borehole Flow
!                3    Fracture and Borehole Flow
!     ISLC(75):  0    
!     ISLC(76):  0    Solute/Species Data in Restart File
!                1    No Solute/Species Data in Restart File
!     ISLC(77):  0    Air solution
!                1    No air solution (Iso-Air)
!     ISLC(78):  0    ECKEChem solver is Gauss-Jordan Elimination
!                1    ECKEChem solver is LU Decomposition
!                2    ECKEChem solver is improved LU Decomposition
!     ISLC(79):  0    No memory check
!                1    Memory check
!     ISLC(80):  0    No PEST output
!                1    EGS Collab Experiment 1 Output for STOMP-GT
!                2    CO2 Mineralization Wallula Output for STOMP-CO2
!     ISLC(81):  0    Solute/Species Transport On
!                1    Solute/Species Transport Off
!     ISLC(82):  0    Hydrate saturation dependent ksP
!                1    Hydrate saturation dependent sP, independent ks
!                2    Hydrate saturation dependent ks, independent sP
!                3    Hydrate saturation independent ksP
!     ISLC(83):  #    Time-step for matrix output for ISLC(34)
!     ISLC(84):  #    Iteration for matrix output for ISLC(34)
!     ISLC(85):  #    IREF_GM for matrix output for ISLC(34)
!     ISLC(86):  0    True mineral volume fractions
!                1    Normalized mineral volume fractions    
!     ISLC(87):  0    Read the restart_bf file
!                1    Skip reading the restart_bf file   
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!
      ...
      ELSEIF( INDEX(ADUM(1:),'restart').NE.0 ) THEN
        ...
        ELSE
          ...
          WRITE(IWR,'(A)') '  w/ Static Domain'
        ENDIF
        IF( INDEX(ADUM(1:),'no restart_bf').NE.0 ) THEN
          WRITE(IWR,'(A)') 'Skip Reading the restart_bf File'
          ISLC(87) = 1
        ENDIF
        IF( INDEX(ADUM(1:),'file').NE.0 ) THEN
          ...
          FNRS = FDUM(1:NCHF)
!
!---      Check for additional restart files  ---
!
          CALL CHKCHR(ISTART,ICOMMA,CHDUM,INDX)
          IF( INDX.NE.0 .AND. ISLC(87).EQ.0 ) THEN
            IDFLT = 1
            FDUM = FNRS_BF
            VARB = 'Fracture/Fault and Borehole Restart File Name'
            CALL RDCHR(ISTART,ICOMMA,NCHF,CHDUM,FDUM)
            WRITE(IWR,'(2X,3A)') VARB(1:IVR),': ',FDUM(1:NCHF)
            FNRS_BF = FDUM(1:NCHF)
          ENDIF
...
!
      IF( INDEX(ADUM(1:),'coupled-flow-problem-vector').NE.0 ) THEN
        ...
        ISLC(34) = 1
!
!---    Check for time-step and iteration counts for vector output  ---
!
        CALL CHKINT(ISTART,ICOMMA,CHDUM,INDX)
        IF( INDX.NE.0 ) THEN
          VARB = 'Time step for Vector Output'
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(83) )
          VARB = 'Iteration for Vector Output'
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(84) )
        ENDIF
      ENDIF
!
...
!
      IF( INDEX(ADUM(1:),'coupled-flow-matrix').NE.0 ) THEN
        ...
        ISLC(34) = 2
!
!---    Check for time-step and iteration counts for matrix output  ---
!
        CALL CHKINT(ISTART,ICOMMA,CHDUM,INDX)
        IF( INDX.NE.0 ) THEN
          VARB = 'Time step for Matrix Output'
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(83) )
          VARB = 'Iteration for Matrix Output'
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(84) )
        ENDIF
      ENDIF
!
...
!
      IF( INDEX(ADUM(1:),'geomechanics-problem-vector').NE.0 ) THEN
        ...
        ISLC(34) = 21
!
!---    Check for time-step, iteration and IREF_GM for 
!       vector output  ---
!
        CALL CHKINT(ISTART,ICOMMA,CHDUM,INDX)
        IF( INDX.NE.0 ) THEN
          VARB = 'Time step for Problem Vector Output'
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(83) )
          VARB = 'Iteration for Problem Vector Output'
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(84) )
          VARB = 'IREF_GM for Problem Vector Output'
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(85) )
        ENDIF
      ENDIF
!
...
!
      IF( INDEX(ADUM(1:),'geomechanics-matrix').NE.0 ) THEN
        ...
        ISLC(34) = 22
!
!---    Check for Check for time-step, iteration and IREF_GM
!       for matrix output  ---
!
        CALL CHKINT(ISTART,ICOMMA,CHDUM,INDX)
        IF( INDX.NE.0 ) THEN
          ...
          VARB = 'Iteration for Matrix Output'
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(84) )
          VARB = 'IREF_GM for Vector Output'
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(85) )
        ENDIF
      ENDIF
!
...
!
!---    Check for Check for time-step, iteration and IREF_GM 
!       for solution vector output  ---
!
        CALL CHKINT(ISTART,ICOMMA,CHDUM,INDX)
        IF( INDX.NE.0 ) THEN
          VARB = 'Time step for Solution Vector Output'
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(83) )
          VARB = 'Iteration for Matrix Output'
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(84) )
          VARB = 'IREF_GM for Problem Vector Output'
      ...
      ENDIF
!
!---  Output Fracture/Fault Flow Problem Vector  ---
!
      IF( INDEX(ADUM(1:),'fracture-problem-vector').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Output Fracture Flow Problem Vector'
        ISLC(34) = 31
!
!---    Check for time-step and iteration counts for vector output  ---
!
        ...
        IF( INDX.NE.0 ) THEN
          ...
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(84) )
        ENDIF
      ELSEIF( INDEX(ADUM(1:),'fault-problem-vector').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Output Fault Flow Problem Vector'
        ISLC(34) = 31
!
!---    Check for time-step and iteration counts for vector output  ---
!
      ...
      ENDIF
!
!---  Output Fracture Matrix  ---
!
      IF( INDEX(ADUM(1:),'fracture-matrix').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Output Fracture Flow Matrix'
        ISLC(34) = 32
!
!---    Check for time-step and iteration counts for matrix output  ---
!
        ...
        IF( INDX.NE.0 ) THEN
          ...
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(84) )
        ENDIF
      ELSEIF( INDEX(ADUM(1:),'fault-matrix').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Output Fault Flow Matrix'
        ISLC(34) = 32
!
!---    Check for time-step and iteration counts for matrix output  ---
!
      ...
      ENDIF
!
!---  Output Fracture Solution Vector  ---
!
      IF( INDEX(ADUM(1:),'fracture-solution-vector').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Output Fracture Flow Solution Vector'
        ISLC(34) = 33
!
!---    Check for time-step and iteration counts for vector output  ---
!
        ...
        IF( INDX.NE.0 ) THEN
          ...
          CALL RDINT( ISTART,ICOMMA,CHDUM,ISLC(84) )
        ENDIF
      ELSEIF( INDEX(ADUM(1:),'fault-solution-vector').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Output Fault Flow Solution Vector'
        ISLC(34) = 33
!
!---    Check for time-step and iteration counts for vector output  ---
!
...
!
      IF( INDEX(ADUM(1:),'scaling factors').NE.0 ) THEN
...
!---  PEST Output Option  ---
!
      IF( INDEX(ADUM(1:),'pest').NE.0 .AND.
     &  INDEX(ADUM(1:),'egs collab').NE.0 .AND.
     &  INDEX(ADUM(1:),'exp1').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ PEST Observational Output against ' // 
     &    'Observational Data from EGS Collab Experiment 1'     
        WRITE(IWR,'(A)') '    E1-PB Flow Rate, ml/min'
        WRITE(IWR,'(A)') '    E1-PI Flow Rate, ml/min'
        WRITE(IWR,'(A)') '    E1-I Pressure, MPa'
        WRITE(IWR,'(A)') '    Hydraulic Fracture Pressure, MPa'
        WRITE(IWR,'(A)') '    E1-PB Temperature, ˚C'
        WRITE(IWR,'(A)') '    E1-PI Temperature, ˚C'
        ISLC(80) = 1
      ELSEIF( INDEX(ADUM(1:),'pest').NE.0 .AND.
     &  INDEX(ADUM(1:),'wallula').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ PEST Observational Output against ' // 
     &    'Observational Data from CO2 Mineralization at Wallula'     
        WRITE(IWR,'(A)') '    Well Pressure Differential, MPa'
        ISLC(80) = 2
      ENDIF
!
...
!---  Memory Check Option  ---
!
      IF( INDEX(ADUM(1:),'memory').NE.0 .AND.
     &  INDEX(ADUM(1:),'check').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Memory Checking'
        ISLC(79) = 1
      ELSE
        WRITE(IWR,'(A)') '  w/ No Memory Checking'
      ENDIF
!
...
!
      ...
      IF( INDEX(CHDUM(1:),'scaling').NE.0 ) ISLC(14) = ISLC(14) + 10
      IF( INDEX(CHDUM(1:),'relative').NE.0 ) ISLC(14) = ISLC(14) + 100
...
!
      IF( INDEX(ADUM(1:),'kspsetfromoptions').NE.0 ) THEN
        WRITE(IWR,'(A)') 'PETSc KSP Convergence Parameters'
        CALL RDINPL( CHDUM )
        CALL LCASE( CHDUM )
        ISTART = 1
        VARB = 'Relative Tolerance'
        IDFLT = 1
        RTOL_PETSC = 1.D-5
        CALL RDDPR(ISTART,ICOMMA,CHDUM,RTOL_PETSC)
        WRITE(IWR,'(2X,A,1PE11.4)') VARB(1:IVR),RTOL_PETSC
        VARB = 'Absolute Tolerance'
        IDFLT = 1
        ATOL_PETSC = 1.D-50
        CALL RDDPR(ISTART,ICOMMA,CHDUM,ATOL_PETSC)
        WRITE(IWR,'(2X,A,1PE11.4)') VARB(1:IVR),ATOL_PETSC
        VARB = 'Differential Tolerance'
        IDFLT = 1
        DTOL_PETSC = 1.D+5
        CALL RDDPR(ISTART,ICOMMA,CHDUM,DTOL_PETSC)
        WRITE(IWR,'(2X,A,1PE11.4)') VARB(1:IVR),DTOL_PETSC
        VARB = 'Maximum Number of Iterations'
        IDFLT = 1
        MAXITS_PETSC = 10000
        CALL RDINT(ISTART,ICOMMA,CHDUM,MAXITS_PETSC)
        WRITE(IWR,'(2X,A,I6)') VARB(1:IVR),MAXITS_PETSC
      ENDIF
...
!
      ...
      INAPL = 0
!
!---  Standard or normalized mineral volume fractions  ---
!
      IF( (INDEX(ADUM(1:),'normalized').NE.0 .OR.
     &  INDEX(ADUM(1:),'muller').NE.0) .AND. 
     &  INDEX(ADUM(1:),'mineral').NE.0 ) ISLC(86) = 1
!
...
!
     ...
     &  INDEX(ADUM(1:),'n2').NE.0 .AND. I_INH.NE.0 ) THEN
        ...
        IGAS = 1
        IAQU = 1
        WRITE(IWR,'(A)') 'Operational Mode: STOMP-HYD-O2-CH4-N2'
          IF( INDEX(ADUM(1:),'iso-o2').NE.0 .AND.
     ...
     &      INDEX(ADUM(1:),'iso-ch4').NE.0 ) THEN
            ...
            ISLC(45) = 1
            ISLC(64) = 1
            WRITE(IWR,'(A)') '  w/ Iso-O2 Option'
            WRITE(IWR,'(A)') '  w/ Iso-CH4 Option'
          ELSEIF( INDEX(ADUM(1:),'iso-o2').NE.0 .AND.
     &      INDEX(ADUM(1:),'iso-n2').NE.0 ) THEN
            IEQT = 1
            IEQW = 2
            IEQA = 0
     ...
     &      INDEX(ADUM(1:),'iso-n2').NE.0 ) THEN
            ...
            WRITE(IWR,'(A)') '  w/ Iso-CH4 Option'
            WRITE(IWR,'(A)') '  w/ Iso-N2 Option'
          ELSEIF( INDEX(ADUM(1:),'iso-o2').NE.0 ) THEN
            IEQT = 1
            IEQW = 2
            IEQA = 0
            IEQO = 3
...
!        NEQ = 5
        ...
        INAPL = 1
        IOM = 34
        WRITE(IWR,'(A)') 'Operational Mode: STOMP-SEQ'
        IF( INDEX(ADUM(1:),'isotherm').NE.0 ) THEN
          ISLC(30) = 1
          IEQT = 0
          IEQW = 1
...
!          IEQS = 4
          ...
          NEQ = NEQ-1
          WRITE(IWR,'(A)') '  w/ Isothermal Option'
!          IF( INDEX(ADUM(1:),'iso-air').NE.0 ) THEN
!            ISLC(77) = 1
!            IEQT = 0
!            IEQW = 1
!            IEQA = 2
...
!          ENDIF
        ENDIF
        IF( INDEX(ADUM(1:),'isobrine').NE.0 ) THEN
          IF( INDEX(ADUM(1:),'simplified-spycher').NE.0 ) THEN
            ISLC(32) = 2
            WRITE(IWR,'(A)') '  w/ Isobrine Option'
            WRITE(IWR,'(A)') '  w/ Simplified Spycher Equilibria Option'
          ELSEIF( INDEX(ADUM(1:),'low-temp-spycher').NE.0 ) THEN
            ISLC(32) = 3
            WRITE(IWR,'(A)') '  w/ Isobrine Option'
            WRITE(IWR,'(A)') '  w/ Low-Temp Spycher Equilibria Option'
          ELSE
            ISLC(32) = 1
            WRITE(IWR,'(A)') '  w/ Isobrine Option'
          ENDIF
...
!
     ...
     &      'Boundary Condition Scheme'
        ...
        IF( INDEX(ADUM(1:),'isotherm').NE.0 ) THEN
          ...
          WRITE(IWR,'(A)') '  w/ Isothermal Option'
        ENDIF
        IF( INDEX(ADUM(1:),'isoair').NE.0 ) THEN
          ISLC(37) = 1
          IEQA = 0
          IEQS = IEQS - 1
          NEQ = NEQ-1
          WRITE(IWR,'(A)') '  w/ Isoair Option'
        ENDIF
        IF( INDEX(ADUM(1:),'hot-dry-rock').NE.0 ) THEN
          ISLC(38) = 1
          WRITE(IWR,'(A)') '  w/ Hot-Dry-Rock Option'
        ENDIF
        IF( INDEX(ADUM(1:),'isobrine').NE.0 ) THEN
          ISLC(32) = 1
          IEQS = 0
        ...
        IF( NEQ.LE.0 ) THEN
          INDX = 4
          NCH = INDEX( ADUM(1:),'  ' )-1
          CHMSG = 'No Solved Equations: ' // ADUM(1:NCH)
          CALL WRMSGS( INDX )
        ENDIF
        ISVC = NEQ
        ISVF = 2*NEQ + 1
...
!
      ...
      ENDIF
!
!---  Hydrate Saturation ksP Options  ---
!
      IF( INDEX(ADUM(1:),'independent').NE.0 .AND.
     &  INDEX(ADUM(1:),'hydrate').NE.0 ) THEN
        IF( INDEX(ADUM(1:),'ksp').NE.0 ) THEN
          ISLC(82) = 3
          WRITE(IWR,'(A)') 'Hydrate Saturation Independent ksP'
        ELSEIF( INDEX(ADUM(1:),'sp').NE.0 ) THEN
          ISLC(82) = 2
          WRITE(IWR,'(A)') 'Hydrate Saturation Independent sP'
        ELSEIF( INDEX(ADUM(1:),'ks').NE.0 ) THEN
          ISLC(82) = 1
          WRITE(IWR,'(A)') 'Hydrate Saturation Independent ks'
        ENDIF
      ENDIF
    
!
!---  Fracture/Fault Geometry Output Options  ---
!
      IF( INDEX(ADUM(1:),'fracture output').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Full Fracture Geometry Output'
        ISLC(35) = 1
      ELSEIF( INDEX(ADUM(1:),'fault output').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Full Fault Geometry Output'
        ISLC(35) = 1
      ENDIF
!
!---  Borehole Geometry Output Options  ---
!
      IF( INDEX(ADUM(1:),'borehole output').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Full Borehole Geometry Output'
        ISLC(36) = 1
      ENDIF
!
!---  Solute/Species Data in Restart File Options  ---
!
      IF( INDEX(ADUM(1:),'no solute/species').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/o Solute/Species Data in Restart File'
        ISLC(76) = 1
      ENDIF
!
!---  ECKEChem solver  ---
!
      IF( INDEX(ADUM(1:),'eckechem-lu').NE.0  ) THEN
        WRITE(IWR,'(A)') '  ECKEChem Solver is LU Decomposition'
        ISLC(78) = 1
      ELSEIF( INDEX(ADUM(1:),'eckechem-fp').NE.0  ) THEN
        INDX = 4
        CHMSG = 'ECECKEChem Full Pivoting Solver is Currently ' // 
     &    'Unavailable'
        CALL WRMSGS( INDX )
!        WRITE(IWR,'(A)') '  ECKEChem Solver is Full Pivoting'
!        ISLC(78) = 2
      ELSE
        WRITE(IWR,'(A)') '  ECKEChem Solver is Gauss-Jordan Elimination'
        ISLC(78) = 0
      ENDIF
!
!---  Fracture and Borehole Flow Options  ---
!
      IF( INDEX(ADUM(1:),'fracture flow').NE.0 .AND.
     &  INDEX(ADUM(1:),'borehole flow').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Fracture and Borehole Flow'
        ISLC(74) = 3
      ELSEIF( INDEX(ADUM(1:),'borehole flow').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Borehole Flow'
        ISLC(74) = 2
      ELSEIF( INDEX(ADUM(1:),'fracture flow').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Fracture Flow'
        ISLC(74) = 1
      ELSEIF( INDEX(ADUM(1:),'fault flow').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ Fault Flow'
        ISLC(74) = 1
      ENDIF
!
...
!
     ...
     &  INDEX(ADUM(1:),'quartile').NE.0 ) THEN
        WRITE(IWR,'(A)') '  w/ First Quartile Convergence'
        ISLC(39) = 1
      ENDIF
!
!---  Reactive Transport Options  ---
...
!
        ...
        ENDIF
!
!---    Check for additional input of a minimum time step + units 
!       and a time-step cut factor  ---
!
        VARB = 'Minimum Time Step + Units & Cut Factor'
        CALL CHKDPR(ISTART,ICOMMA,CHDUM,INDX)
        IF( INDX.EQ.1 ) THEN
          ISTX = ISTART
          ICMX = ICOMMA
...
!
        ...
        ENDIF
!
!---    Check for additional input of an on/off switch for solute
!       transport  ---
!
        VARB = 'On/Off Switch for Solute Transport'
        CALL CHKCHR(ISTART,ICOMMA,CHDUM,INDX)
        IF( INDX.EQ.1 ) THEN
          CALL RDCHR(ISTART,ICOMMA,NCH,CHDUM,CDUM)
          IF( INDEX(CDUM(1:),'transport').NE.0 .AND.
     &      INDEX(CDUM(1:),'off').NE.0 ) THEN
            ITOFF(N) = 1
          ENDIF
        ENDIF
  100 CONTINUE
...
!
     ...
     &      UNTS(1:NCH),': ',DFNWC
        ...
        ENDIF
!
!---  STOMP-SEQ or STOMP-EOR  ---
!
      ELSEIF( IOM.EQ.34 .OR. IOM.EQ.43 ) THEN
        CALL RDINPL( CHDUM )
        CALL LCASE( CHDUM )
        ISTART = 1
        VARB = 'Nonaqueous-Liquid Diffusion Option'
