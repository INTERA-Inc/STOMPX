...
!
              ...
              CALL lis_vector_set_value_f( 1,NMD,BUFFER,
     &          G_RHS_VEC,IERR )
...
!
            ...
            CALL lis_vector_set_value_f( 0,NMD,BUFFER,
     &        G_RHS_VEC,IERR )
...
!---  Lis solver  ---
!
      CALL lis_matrix_set_csr_f( NNZ_GM,NLU_GM,MLU_GM,DLU,G_MAT,IERR )
      IF( IERR.NE.0 ) THEN
        I_ERR(1) = 0
        I_ERR(2) = 0
        I_ERR(3) = 0
        I_ERR(4) = ID
        M_ERR(1) = 'Geomechanics Lis Matrix Set CSR'
        RETURN
      ENDIF
...
!     more memory is needed for geomechanics ---
!
      IF( NUKGL(ID+1).GT.NUKFL(ID+1) ) THEN     
        IF( ALLOCATED(BLU) ) THEN
          DEALLOCATE( BLU,STAT=ISTAT )
          CHMSG = 'BLU'
          CALL DEALLOC_ERROR( CHMSG,ISTAT )
        ENDIF
        ALLOCATE( BLU(1:NUKGL(ID+1)),STAT=ISTAT )
        CHMSG = 'BLU'
        CALL ALLOC_ERROR( CHMSG,ISTAT )
      ENDIF
...
!
      ...
      INDX = 2
      CALL STOMP_LIS_SOLVE( G_KSP,G_MAT,G_RHS_VEC,G_SOL_VEC,
     &  NUKGO(ID+1),NUKGL(ID+1),INDX )
...
!
!---  Displacements at FE ghost nodes  ---
!
      CALL DISP_GN_GM
