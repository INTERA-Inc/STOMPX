...
!
     ...
     &  INDEX(ADUM(1:),'vertices').NE.0 ) THEN
        ICS = 11
        WRITE(IWR,'(/,A)') 'KIJ Vertices Grid Input'
        WRITE(IWR,'(/,A)') 'Orthogonal Boundary Fitted ' //
     &    'Coordinate System'
      ELSEIF( INDEX(ADUM(1:),'jki').NE.0 .AND.
     &  INDEX(ADUM(1:),'vertices').NE.0 ) THEN
        ICS = 12
        WRITE(IWR,'(/,A)') 'JKI Vertices Grid Input'
        WRITE(IWR,'(/,A)') 'Orthogonal Boundary Fitted ' //
     &    'Coordinate System'
      ELSEIF( INDEX(ADUM(1:),'ijk').NE.0 .AND.
     &  INDEX(ADUM(1:),'vertices').NE.0 ) THEN
        ICS = 13
        WRITE(IWR,'(/,A)') 'IJK Vertices Grid Input'
        WRITE(IWR,'(/,A)') 'Orthogonal Boundary Fitted ' //
     &    'Coordinate System'
...
!
      IF( ISLC(67).EQ.1 ) THEN
        ...
        CALL RDINT(ISTART,ICOMMA,CHDUM,IP_MPI)
        IF( IP_MPI.GT.IFLD  ) THEN
          INDX = 4
          CHMSG = 'IP_MPI > IFLD'
          CALL WRMSGS( INDX )
        ENDIF
        VARB = 'Number of J-indexed Processors'
        CALL RDINT(ISTART,ICOMMA,CHDUM,JP_MPI)
        IF( JP_MPI.GT.JFLD  ) THEN
          INDX = 4
          CHMSG = 'JP_MPI > JFLD'
          CALL WRMSGS( INDX )
        ENDIF
        VARB = 'Number of K-indexed Processors'
        CALL RDINT(ISTART,ICOMMA,CHDUM,KP_MPI)
        IF( KP_MPI.GT.KFLD  ) THEN
          INDX = 4
          CHMSG = 'KP_MPI > KFLD'
          CALL WRMSGS( INDX )
        ENDIF
        NP_MPI = IP_MPI*JP_MPI*KP_MPI
!
!---    Write MPI processor counts  ---
!
        WRITE (IWR,'(/,A)') 'Processor System Dimensions'
        WRITE (IWR,'(2X,A,I9)') 'Number of I Processors:  ',IP_MPI
        WRITE (IWR,'(2X,A,I9)') 'Number of J Processors:  ',JP_MPI
        WRITE (IWR,'(2X,A,I9)') 'Number of K Processors:  ',KP_MPI
        WRITE (IWR,'(2X,A,I9)') 'Number Processors:  ',NP_MPI
!
...
!
        ...
        IF( IP_MPI.EQ.1 ) THEN
          ID_MPI(1,IP_MPI) = 1
          ID_MPI(2,IP_MPI) = IFLD
        ENDIF
        IF( JP_MPI.EQ.1 ) THEN
          JD_MPI(1,JP_MPI) = 1
          JD_MPI(2,JP_MPI) = JFLD
        ENDIF
        IF( KP_MPI.EQ.1 ) THEN
          KD_MPI(1,KP_MPI) = 1
          KD_MPI(2,KP_MPI) = KFLD
        ENDIF
      ENDIF
!
...
!
      ELSEIF( ICS.EQ.14 ) THEN
        CALL RDKJIVERT
!
!---  Grids defined through an unstructured grid specification file  ---
!
      ELSEIF( ICS.EQ.15 ) THEN
        CALL RDGSF
!
!---  Grids defined through LaGrit AVS ASCII UCD file  ---
!
      ELSEIF( ICS.EQ.17 ) THEN
        CALL RDAVSUCD
!
!---  Grids defined through a MODFLOW Discretization file  ---
!
...
!
     ...
     &   .OR. ICS.EQ.10 .OR. ICS.EQ.11 .OR. ICS.EQ.12
     &   .OR. ICS.EQ.13 .OR. ICS.EQ.14 .OR. ICS.EQ.15
     &   .OR. ICS.EQ.16 .OR. ICS.EQ.17 ) THEN
        CALL AVHEX
        ICS = 3
      ENDIF
!
...
!
      NC = 1
      IF( (IJFLD.LE.JKFLD.AND.IJFLD.LE.KIFLD) .OR. ISLC(67).EQ.1 ) THEN
!
!---    X-Y Plane yields the lowest band width or parallel preprocessing
...
!----------------------Description-------------------------------------!
!
!     Read input file for grid rotation information.
!     This card is used to align fracture surface normals with
!     the alignment of the principal stress directions.
!     This rotation will be required when the grid is rotated to align
!     the grid axis to be aligned with the principal stress directions
!     for the geomechanical calculations. The card reads up to three
!     rotations through three Euler angles
!
!     alpha - x or x' directions
!     beta - y or y' directions
!     gamma - z or z' directions
!
!     Rotations are carried out in the order input.
!     
!     The output from this read is a rotation matrix, which
!     is stored in the array ROTMAT_GM(3,3)
!
!----------------------Authors-----------------------------------------!
!
!     Written by Mark White, PNNL, 30 December 2020.
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!
      ISUB_LOG = ISUB_LOG+1
      SUB_LOG(ISUB_LOG) = '/RDROTATION'
!
!---  Write card information to ouput file  ---
!
      CARD = 'Grit Rotation Card'
      ICD = INDEX( CARD,'  ' )-1
      WRITE (IWR,'(//,3A)') ' ~ ',CARD(1:ICD),': '
!
...
!
      ...
      CALL LCASE( CHDUM )
      ISTART = 1
      VARB = 'Number of Rotations'
      CALL RDINT(ISTART,ICOMMA,CHDUM,NROT)
      WRITE (IWR,'(4X,A,I9)') 'Number of Rotations:   ',NROT
      DO NR = 1,NROT
        CALL RDINPL( CHDUM )
        CALL LCASE( CHDUM )
        ISTART = 1
        ...
        WRITE(IWR,'(6X,2A,$)') VARB(1:IVR),', '
        VARB = 'Rotation Angle Units'
        CALL RDCHR(ISTART,ICOMMA,NCH,CHDUM,UNTS)        
        WRITE(IWR,'(2A,1PE11.4,$)') UNTS(1:NCH),': ',THETA
        INDX = 0
        CALL RDUNIT(UNTS,THETA,INDX)
        WRITE(IWR,'(A,1PE11.4,A)') ' (',THETA,', deg)'
        VARB = 'Euler Angle'
        CALL RDCHR(ISTART,ICOMMA,NCH,CHDUM,ADUM)
        IF( INDEX(ADUM(1:),'alpha').NE.0 ) THEN
          RM2(1,1) = 1.D+0
          RM2(1,2) = 0.D+0
          RM2(1,3) = 0.D+0
          RM2(2,1) = 0.D+0
          ...
          DO N = 1,3
            ...
            ENDDO
          ENDDO
        ELSEIF( INDEX(ADUM(1:),'beta').NE.0 ) THEN
          RM2(1,1) = COS(THETA)
          RM2(1,2) = 0.D+0
          RM2(1,3) = SIN(THETA)
          RM2(2,1) = 0.D+0
          ...
          DO N = 1,3
            ...
            ENDDO
          ENDDO
        ELSEIF( INDEX(ADUM(1:),'gamma').NE.0 ) THEN
          RM2(1,1) = COS(THETA)
          RM2(1,2) = -SIN(THETA)
          RM2(1,3) = 0.D+0
          RM2(2,1) = SIN(THETA)
        ...
        ELSE
          INDX = 4
          CHMSG = 'Unrecognized Euler Angle Option: ' // ADUM
          CALL WRMSGS( INDX )
        ENDIF
      ENDDO
!
...
!
        ...
        DO I = 1,IFLD
          N = ND(I,J,K)
!
!---      Check for nodes with zero volume and skip those nodes  ---
!
          SUMX = ABS(XE(2,N)-XE(1,N)) + ABS(XE(4,N)-XE(3,N)) + 
     &      ABS(XE(6,N)-XE(5,N)) + ABS(XE(8,N)-XE(7,N))
          SUMY = ABS(YE(3,N)-YE(1,N)) + ABS(YE(4,N)-YE(2,N)) + 
     &      ABS(YE(7,N)-YE(5,N)) + ABS(YE(8,N)-YE(6,N))
          SUMZ = ABS(ZE(5,N)-ZE(1,N)) + ABS(ZE(6,N)-ZE(2,N)) + 
     &      ABS(ZE(7,N)-ZE(3,N)) + ABS(ZE(8,N)-ZE(4,N))
          IF( SUMX*SUMY*SUMZ.LT.EPSL ) CYCLE
          NB = N-IJFLD
          NS = N-IFLD
          NW = N-1
          NPX = NSX(N)
...
!
          ...
          IF( I.EQ.1 .OR. INBS(3,N).GT.0 ) THEN
            DXGP(NPX) = DXW
          ELSE
            DXGP(NPX) = DXGP(NPX) + DXW
          ENDIF
          IF( I.EQ.IFLD .OR. INBS(4,N).GT.0 ) THEN
            DXGP(NQX) = DXE
          ELSE
...
!
          ...
          IF( J.EQ.1 .OR. INBS(2,N).GT.0 ) THEN
            DYGP(NPY) = DYS
          ELSE
            DYGP(NPY) = DYGP(NPY) + DYS
          ENDIF
          IF( J.EQ.JFLD .OR. INBS(5,N).GT.0 ) THEN
            DYGP(NQY) = DYN
          ELSE
...
!
          ...
          IF( K.EQ.1 .OR. INBS(1,N).GT.0 ) THEN
            DZGP(NPZ) = DZB
          ELSE
            DZGP(NPZ) = DZGP(NPZ) + DZB
          ENDIF
          IF( K.EQ.KFLD .OR. INBS(6,N).GT.0 ) THEN
            DZGP(NQZ) = DZT
          ELSE
...
!
     ...
     &  (ICS.EQ.3) .OR. (ISLC(63).EQ.1) ) THEN
        DO 200 N = 1,NFBN+NWN_LW
          IF( IXP(N).EQ.0 .OR. IBR(4,N).NE.N ) GOTO 200
          IF( N.GT.NFLD .AND. LBR.GT.0 ) THEN
            NF = INP(N-NFLD)
          ELSE
            NF = N
          ENDIF
...
!
          NW = ICM(1,3,N)
          IF( NW.GT.0 ) THEN
            IF( NW.GT.NFLD .AND. LBR.GT.0 ) THEN
              NFWX = INP(NW-NFLD)
            ELSE
              NFWX = NW
            ENDIF
            IRY = IBR(2,NFWX)/(IBR(2,NF)+1) + 1
            IRZ = IBR(3,NFWX)/(IBR(3,NF)+1) + 1
            NC = NC0X(IRY,IRZ)
            NW = ICM(NC,3,N)
...
!
            NWS = ICM(1,2,NW)
            IF( NWS.GT.0 ) THEN
              IF( NWS.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFWSX = INP(NWS-NFLD)
              ELSE
                NFWSX = NWS
              ENDIF
              IRX = IBR(1,NFWSX)/(IBR(1,NFWX)+1) + 1
              IRZ = IBR(3,NFWSX)/(IBR(3,NFWX)+1) + 1
              NC = NC3X(IRX,IRZ)
              NWS = ICM(NC,2,NW)
...
!
              NWSB = ICM(1,1,NWS)
              IF( NWSB.GT.0 ) THEN
                IF( NWSB.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFWSB = INP(NWSB-NFLD)
                ELSE
                  NFWSB = NWSB
                ENDIF
                IRX = IBR(1,NFWSB)/(IBR(1,NFWSX)+1) + 1
                IRY = IBR(2,NFWSB)/(IBR(2,NFWSX)+1) + 1
                NC = NC1X(IRX,IRY)
                NWSB = ICM(NC,1,NWS)
...
!
          NS = ICM(1,2,N)
          IF( NS.GT.0 ) THEN
            IF( NS.GT.NFLD .AND. LBR.GT.0 ) THEN
              NFSX = INP(NS-NFLD)
            ELSE
              NFSX = NS
            ENDIF
            IRX = IBR(1,NFSX)/(IBR(1,NF)+1) + 1
            IRZ = IBR(3,NFSX)/(IBR(3,NF)+1) + 1
            NC = NC0X(IRX,IRZ)
            NS = ICM(NC,2,N)
...
!
            NSB = ICM(1,1,NS)
            IF( NSB.GT.0 ) THEN
              IF( NSB.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFSBX = INP(NSB-NFLD)
              ELSE
                NFSBX = NSB
              ENDIF
              IRX = IBR(1,NFSBX)/(IBR(1,NFSX)+1) + 1
              IRY = IBR(2,NFSBX)/(IBR(2,NFSX)+1) + 1
              NC = NC2X(IRX,IRY)
              NSB = ICM(NC,1,NS)
...
!
              NSBW = ICM(1,3,NSB)
              IF( NSBW.GT.0 ) THEN
                IF( NSBW.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFSBW = INP(NSBW-NFLD)
                ELSE
                  NFSBW = NSBW
                ENDIF
                IRY = IBR(2,NFSBW)/(IBR(2,NFSBX)+1) + 1
                IRZ = IBR(3,NFSBW)/(IBR(3,NFSBX)+1) + 1
                NC = NC1X(IRY,IRZ)
                NSBW = ICM(NC,3,NSB)
...
!
          NB = ICM(1,1,N)
          IF( NB.GT.0 ) THEN
            IF( NB.GT.NFLD .AND. LBR.GT.0 ) THEN
              NFBX = INP(NB-NFLD)
            ELSE
              NFBX = NB
            ENDIF
            IRX = IBR(1,NFBX)/(IBR(1,NF)+1) + 1
            IRY = IBR(2,NFBX)/(IBR(2,NF)+1) + 1
            NC = NC0X(IRX,IRY)
            NB = ICM(NC,1,N)
...
!
            NBW = ICM(1,3,NB)
            IF( NBW.GT.0 ) THEN
              IF( NBW.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFBWX = INP(NBW-NFLD)
              ELSE
                NFBWX = NBW
              ENDIF
              IRY = IBR(2,NFBWX)/(IBR(2,NFBX)+1) + 1
              IRZ = IBR(3,NFBWX)/(IBR(3,NFBX)+1) + 1
              NC = NC2X(IRY,IRZ)
              NBW = ICM(NC,3,NB)
...
!
              NBWS = ICM(1,2,NBW)
              IF( NBWS.GT.0 ) THEN
                IF( NBWS.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFBWS = INP(NBWS-NFLD)
                ELSE
                  NFBWS = NBWS
                ENDIF
                IRX = IBR(1,NFBWS)/(IBR(1,NFBWX)+1) + 1
                IRZ = IBR(3,NFBWS)/(IBR(3,NFBWX)+1) + 1
                NC = NC1X(IRX,IRZ)
                NBWS = ICM(NC,2,NBW)
...
!
          NE = ICM(1,4,N)
          IF( NE.GT.0 ) THEN
            IF( NE.GT.NFLD .AND. LBR.GT.0 ) THEN
              NFEX = INP(NE-NFLD)
            ELSE
              NFEX = NE
            ENDIF
            IRY = IBR(2,NFEX)/(IBR(2,NF)+1) + 1
            IRZ = IBR(3,NFEX)/(IBR(3,NF)+1) + 1
            NC = NC0X(IRY,IRZ)
            NE = ICM(NC,4,N)
...
!
            NES = ICM(1,2,NE)
            IF( NES.GT.0 ) THEN
              IF( NES.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFESX = INP(NES-NFLD)
              ELSE
                NFESX = NES
              ENDIF
              IRX = IBR(1,NFESX)/(IBR(1,NFEX)+1) + 1
              IRZ = IBR(3,NFESX)/(IBR(3,NFEX)+1) + 1
              NC = NC0X(IRX,IRZ)
              NES = ICM(NC,2,NE)
...
!
              NESB = ICM(1,1,NES)
              IF( NESB.GT.0 ) THEN
                IF( NESB.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFESB = INP(NESB-NFLD)
                ELSE
                  NFESB = NESB
                ENDIF
                IRX = IBR(1,NFESB)/(IBR(1,NFESX)+1) + 1
                IRY = IBR(2,NFESB)/(IBR(2,NFESX)+1) + 1
                NC = NC2X(IRX,IRY)
                NESB = ICM(NC,1,NES)
...
!
              NSBE = ICM(1,4,NSB)
              IF( NSBE.GT.0 ) THEN
                IF( NSBE.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFSBE = INP(NSBE-NFLD)
                ELSE
                  NFSBE = NSBE
                ENDIF
                IRY = IBR(2,NFSBE)/(IBR(2,NFSBX)+1) + 1
                IRZ = IBR(3,NFSBE)/(IBR(3,NFSBX)+1) + 1
                NC = NC1X(IRY,IRZ)
                NSBE = ICM(NC,4,NSB)
...
!
            NBE = ICM(1,4,NB)
            IF( NBE.GT.0 ) THEN
              IF( NBE.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFBEX = INP(NBE-NFLD)
              ELSE
                NFBEX = NBE
              ENDIF
              IRY = IBR(2,NFBEX)/(IBR(2,NFBX)+1) + 1
              IRZ = IBR(3,NFBEX)/(IBR(3,NFBX)+1) + 1
              NC = NC2X(IRY,IRZ)
              NBE = ICM(NC,4,NB)
...
!
              NBES = ICM(1,2,NBE)
              IF( NBES.GT.0 ) THEN
                IF( NBES.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFBES = INP(NBES-NFLD)
                ELSE
                  NFBES = NBES
                ENDIF
                IRX = IBR(1,NFBES)/(IBR(1,NFBEX)+1) + 1
                IRZ = IBR(3,NFBES)/(IBR(3,NFBEX)+1) + 1
                NC = NC2X(IRX,IRZ)
                NBES = ICM(NC,2,NBE)
...
!
            NEN = ICM(1,5,NE)
            IF( NEN.GT.0 ) THEN
              IF( NEN.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFENX = INP(NEN-NFLD)
              ELSE
                NFENX = NEN
              ENDIF
              IRX = IBR(1,NFENX)/(IBR(1,NFEX)+1) + 1
              IRZ = IBR(3,NFENX)/(IBR(3,NFEX)+1) + 1
              NC = NC0X(IRX,IRZ)
              NEN = ICM(NC,5,NE)
...
!
              NENB = ICM(1,1,NEN)
              IF( NENB.GT.0 ) THEN
                IF( NENB.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFENB = INP(NENB-NFLD)
                ELSE
                  NFENB = NENB
                ENDIF
                IRX = IBR(1,NFENB)/(IBR(1,NFENX)+1) + 1
                IRY = IBR(2,NFENB)/(IBR(2,NFENX)+1) + 1
                NC = NC0X(IRX,IRY)
                NENB = ICM(NC,1,NEN)
...
!
          NN = ICM(1,5,N)
          IF( NN.GT.0 ) THEN
            IF( NN.GT.NFLD .AND. LBR.GT.0 ) THEN
              NFNX = INP(NN-NFLD)
            ELSE
              NFNX = NN
            ENDIF
            IRX = IBR(1,NFNX)/(IBR(1,NF)+1) + 1
            IRZ = IBR(3,NFNX)/(IBR(3,NF)+1) + 1
            NC = NC3X(IRX,IRZ)
            NN = ICM(NC,5,N)
...
!
            NNB = ICM(1,1,NN)
            IF( NNB.GT.0 ) THEN
              IF( NNB.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFNBX = INP(NNB-NFLD)
              ELSE
                NFNBX = NNB
              ENDIF
              IRX = IBR(1,NFNBX)/(IBR(1,NFNX)+1) + 1
              IRY = IBR(2,NFNBX)/(IBR(2,NFNX)+1) + 1
              NC = NC3X(IRX,IRY)
              NNB = ICM(NC,1,NN)
...
!
              NNBE = ICM(1,4,NNB)
              IF( NNBE.GT.0 ) THEN
                IF( NNBE.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFNBE = INP(NNBE-NFLD)
                ELSE
                  NFNBE = NNBE
                ENDIF
                IRY = IBR(2,NFNBE)/(IBR(2,NFNBX)+1) + 1
                IRZ = IBR(3,NFNBE)/(IBR(3,NFNBX)+1) + 1
                NC = NC2X(IRY,IRZ)
                NNBE = ICM(NC,4,NNB)
...
!
              NBEN = ICM(1,5,NBE)
              IF( NBEN.GT.0 ) THEN
                IF( NBEN.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFBEN = INP(NBEN-NFLD)
                ELSE
                  NFBEN = NBEN
                ENDIF
                IRX = IBR(1,NFBEN)/(IBR(1,NFBEX)+1) + 1
                IRZ = IBR(3,NFBEN)/(IBR(3,NFBEX)+1) + 1
                NC = NC2X(IRX,IRZ)
                NBEN = ICM(NC,5,NBE)
...
!
            NWN = ICM(1,5,NW)
            IF( NWN.GT.0 ) THEN
              IF( NWN.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFWNX = INP(NWN-NFLD)
              ELSE
                NFWNX = NWN
              ENDIF
              IRX = IBR(1,NFWNX)/(IBR(1,NFWX)+1) + 1
              IRZ = IBR(3,NFWNX)/(IBR(3,NFWX)+1) + 1
              NC = NC3X(IRX,IRZ)
              NWN = ICM(NC,5,NW)
...
!
              NWNB = ICM(1,1,NWN)
              IF( NWNB.GT.0 ) THEN
                IF( NWNB.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFWNB = INP(NWNB-NFLD)
                ELSE
                  NFWNB = NWNB
                ENDIF
                IRX = IBR(1,NFWNB)/(IBR(1,NFWNX)+1) + 1
                IRY = IBR(2,NFWNB)/(IBR(2,NFWNX)+1) + 1
                NC = NC3X(IRX,IRY)
                NWNB = ICM(NC,1,NWN)
...
!
              NNBW = ICM(1,3,NNB)
              IF( NNBW.GT.0 ) THEN
                IF( NNBW.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFNBW = INP(NNBW-NFLD)
                ELSE
                  NFNBW = NNBW
                ENDIF
                IRY = IBR(2,NFNBW)/(IBR(2,NFNBX)+1) + 1
                IRZ = IBR(3,NFNBW)/(IBR(3,NFNBX)+1) + 1
                NC = NC2X(IRY,IRZ)
                NNBW = ICM(NC,3,NNB)
...
!
              NBWN = ICM(1,5,NBW)
              IF( NBWN.GT.0 ) THEN
                IF( NBWN.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFBWN = INP(NBWN-NFLD)
                ELSE
                  NFBWN = NBWN
                ENDIF
                IRX = IBR(1,NFBWN)/(IBR(1,NFBWX)+1) + 1
                IRZ = IBR(3,NFBWN)/(IBR(3,NFBWX)+1) + 1
                NC = NC1X(IRX,IRZ)
                NBWN = ICM(NC,5,NBW)
...
!
              NWST = ICM(1,6,NWS)
              IF( NWST.GT.0 ) THEN
                IF( NWST.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFWST = INP(NWST-NFLD)
                ELSE
                  NFWST = NWST
                ENDIF
                IRX = IBR(1,NFWST)/(IBR(1,NFWSX)+1) + 1
                IRY = IBR(2,NFWST)/(IBR(2,NFWSX)+1) + 1
                NC = NC1X(IRX,IRY)
                NWST = ICM(NC,6,NWS)
...
!
            NST = ICM(1,6,NS)
            IF( NST.GT.0 ) THEN
              IF( NST.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFSTX = INP(NST-NFLD)
              ELSE
                NFSTX = NST
              ENDIF
              IRX = IBR(1,NFSTX)/(IBR(1,NFSX)+1) + 1
              IRY = IBR(2,NFSTX)/(IBR(2,NFSX)+1) + 1
              NC = NC3X(IRX,IRY)
              NST = ICM(NC,6,NS)
...
!
              NSTW = ICM(1,3,NST)
              IF( NSTW.GT.0 ) THEN
                IF( NSTW.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFSTW = INP(NSTW-NFLD)
                ELSE
                  NFSTW = NSTW
                ENDIF
                IRY = IBR(2,NFSTW)/(IBR(2,NFSTX)+1) + 1
                IRZ = IBR(3,NFSTW)/(IBR(3,NFSTX)+1) + 1
                NC = NC3X(IRY,IRZ)
                NSTW = ICM(NC,3,NST)
...
!
          NT = ICM(1,6,N)
          IF( NT.GT.0 ) THEN
            IF( NT.GT.NFLD .AND. LBR.GT.0 ) THEN
              NFTX = INP(NT-NFLD)
            ELSE
              NFTX = NT
            ENDIF
            IRX = IBR(1,NFTX)/(IBR(1,NF)+1) + 1
            IRY = IBR(2,NFTX)/(IBR(2,NF)+1) + 1
            NC = NC0X(IRX,IRY)
            NT = ICM(NC,6,N)
...
!
            NTW = ICM(1,3,NT)
            IF( NTW.GT.0 ) THEN
              IF( NTW.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFTWX = INP(NTW-NFLD)
              ELSE
                NFTWX = NTW
              ENDIF
              IRY = IBR(2,NFTWX)/(IBR(2,NFTX)+1) + 1
              IRZ = IBR(3,NFTWX)/(IBR(3,NFTX)+1) + 1
              NC = NC0X(IRY,IRZ)
              NTW = ICM(NC,3,NT)
...
!
              NTWS = ICM(1,2,NTW)
              IF( NTWS.GT.0 ) THEN
                IF( NTWS.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFTWS = INP(NTWS-NFLD)
                ELSE
                  NFTWS = NTWS
                ENDIF
                IRX = IBR(1,NFTWS)/(IBR(1,NFTWX)+1) + 1
                IRZ = IBR(3,NFTWS)/(IBR(3,NFTWX)+1) + 1
                NC = NC3X(IRX,IRZ)
                NTWS = ICM(NC,2,NTW)
...
!
              NEST = ICM(1,6,NES)
              IF( NEST.GT.0 ) THEN
                IF( NEST.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFEST = INP(NEST-NFLD)
                ELSE
                  NFEST = NEST
                ENDIF
                IRX = IBR(1,NFEST)/(IBR(1,NFESX)+1) + 1
                IRY = IBR(2,NFEST)/(IBR(2,NFESX)+1) + 1
                NC = NC2X(IRX,IRY)
                NEST = ICM(NC,6,NES)
...
!
              NSTE = ICM(1,4,NST)
              IF( NSTE.GT.0 ) THEN
                IF( NSTE.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFSTE = INP(NSTE-NFLD)
                ELSE
                  NFSTE = NSTE
                ENDIF
                IRY = IBR(2,NFSTE)/(IBR(2,NFSTX)+1) + 1
                IRZ = IBR(3,NFSTE)/(IBR(3,NFSTX)+1) + 1
                NC = NC2X(IRY,IRZ)
                NSTE = ICM(NC,4,NST)
...
!
            NTE = ICM(1,4,NT)
            IF( NTE.GT.0 ) THEN
              IF( NTE.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFTEX = INP(NTE-NFLD)
              ELSE
                NFTEX = NTE
              ENDIF
              IRY = IBR(2,NFTEX)/(IBR(2,NFTX)+1) + 1
              IRZ = IBR(3,NFTEX)/(IBR(3,NFTX)+1) + 1
              NC = NC0X(IRY,IRZ)
              NTE = ICM(NC,4,NT)
...
!
              NTES = ICM(1,2,NTE)
              IF( NTES.GT.0 ) THEN
                IF( NTES.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFTES = INP(NTES-NFLD)
                ELSE
                  NFTES = NTES
                ENDIF
                IRX = IBR(1,NFTES)/(IBR(1,NFTEX)+1) + 1
                IRZ = IBR(3,NFTES)/(IBR(3,NFTEX)+1) + 1
                NC = NC0X(IRX,IRZ)
                NTES = ICM(NC,2,NTE)
...
!
              NENT = ICM(1,6,NEN)
              IF( NENT.GT.0 ) THEN
                IF( NENT.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFENT = INP(NENT-NFLD)
                ELSE
                  NFENT = NENT
                ENDIF
                IRX = IBR(1,NFENT)/(IBR(1,NFENX)+1) + 1
                IRY = IBR(2,NFENT)/(IBR(2,NFENX)+1) + 1
                NC = NC0X(IRX,IRY)
                NENT = ICM(NC,6,NEN)
...
!
            NNT = ICM(1,6,NN)
            IF( NNT.GT.0 ) THEN
              IF( NNT.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFNTX = INP(NNT-NFLD)
              ELSE
                NFNTX = NNT
              ENDIF
              IRX = IBR(1,NFNTX)/(IBR(1,NFNX)+1) + 1
              IRY = IBR(2,NFNTX)/(IBR(2,NFNX)+1) + 1
              NC = NC3X(IRX,IRY)
              NNT = ICM(NC,6,NN)
...
!
              NNTE = ICM(1,4,NNT)
              IF( NNTE.GT.0 ) THEN
                IF( NNTE.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFNTE = INP(NNTE-NFLD)
                ELSE
                  NFNTE = NNTE
                ENDIF
                IRY = IBR(2,NFNTE)/(IBR(2,NFNTX)+1) + 1
                IRZ = IBR(3,NFNTE)/(IBR(3,NFNTX)+1) + 1
                NC = NC0X(IRY,IRZ)
                NNTE = ICM(NC,4,NNT)
...
!
              NTEN = ICM(1,5,NTE)
              IF( NTEN.GT.0 ) THEN
                IF( NTEN.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFTEN = INP(NTEN-NFLD)
                ELSE
                  NFTEN = NTEN
                ENDIF
                IRX = IBR(1,NFTEN)/(IBR(1,NFTEX)+1) + 1
                IRZ = IBR(3,NFTEN)/(IBR(3,NFTEX)+1) + 1
                NC = NC0X(IRX,IRZ)
                NTEN = ICM(NC,5,NTE)
...
!
              NWNT = ICM(1,6,NWN)
              IF( NWNT.GT.0 ) THEN
                IF( NWNT.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFWNT = INP(NWNT-NFLD)
                ELSE
                  NFWNT = NWNT
                ENDIF
                IRX = IBR(1,NFWNT)/(IBR(1,NFWNX)+1) + 1
                IRY = IBR(2,NFWNT)/(IBR(2,NFWNX)+1) + 1
                NC = NC3X(IRX,IRY)
                NWNT = ICM(NC,6,NWN)
...
!
              NNTW = ICM(1,3,NNT)
              IF( NNTW.GT.0 ) THEN
                IF( NNTW.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFNTW = INP(NNTW-NFLD)
                ELSE
                  NFNTW = NNTW
                ENDIF
                IRY = IBR(2,NFNTW)/(IBR(2,NFNTX)+1) + 1
                IRZ = IBR(3,NFNTW)/(IBR(3,NFNTX)+1) + 1
                NC = NC0X(IRY,IRZ)
                NNTW = ICM(NC,3,NNT)
...
!
              NTWN = ICM(1,5,NTW)
              IF( NTWN.GT.0 ) THEN
                IF( NTWN.GT.NFLD .AND. LBR.GT.0 ) THEN
                  NFTWN = INP(NTWN-NFLD)
                ELSE
                  NFTWN = NTWN
                ENDIF
                IRX = IBR(1,NFTWN)/(IBR(1,NFTWX)+1) + 1
                IRZ = IBR(3,NFTWN)/(IBR(3,NFTWX)+1) + 1
                NC = NC3X(IRX,IRZ)
                NTWN = ICM(NC,5,NTW)
...
!
            NSW = ICM(1,3,NS)
            IF( NSW.GT.0 ) THEN
              IF( NSW.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFSWX = INP(NSW-NFLD)
              ELSE
                NFSWX = NSW
              ENDIF
              IRY = IBR(2,NFSWX)/(IBR(2,NFSX)+1) + 1
              IRZ = IBR(3,NFSWX)/(IBR(3,NFSX)+1) + 1
              NC = NC1X(IRY,IRZ)
              NSW = ICM(NC,3,NS)
...
!
            NSE = ICM(1,4,NS)
            IF( NSE.GT.0 ) THEN
              IF( NSE.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFSEX = INP(NSE-NFLD)
              ELSE
                NFSEX = NSE
              ENDIF
              IRY = IBR(2,NFSEX)/(IBR(2,NFSX)+1) + 1
              IRZ = IBR(3,NFSEX)/(IBR(3,NFSX)+1) + 1
              NC = NC1X(IRY,IRZ)
              NSE = ICM(NC,4,NS)
...
!
            NNE = ICM(1,4,NN)
            IF( NNE.GT.0 ) THEN
              IF( NNE.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFNEX = INP(NNE-NFLD)
              ELSE
                NFNEX = NNE
              ENDIF
              IRY = IBR(2,NFNEX)/(IBR(2,NFNX)+1) + 1
              IRZ = IBR(3,NFNEX)/(IBR(3,NFNX)+1) + 1
              NC = NC2X(IRY,IRZ)
              NNE = ICM(NC,4,NN)
...
!
            NNW = ICM(1,3,NN)
            IF( NNW.GT.0 ) THEN
              IF( NNW.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFNWX = INP(NNW-NFLD)
              ELSE
                NFNWX = NNW
              ENDIF
              IRY = IBR(2,NFNWX)/(IBR(2,NFNX)+1) + 1
              IRZ = IBR(3,NFNWX)/(IBR(3,NFNX)+1) + 1
              NC = NC2X(IRY,IRZ)
              NNW = ICM(NC,3,NN)
...
!
            NBS = ICM(1,2,NB)
            IF( NBS.GT.0 ) THEN
              IF( NBS.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFBSX = INP(NBS-NFLD)
              ELSE
                NFBSX = NBS
              ENDIF
              IRX = IBR(1,NFBSX)/(IBR(1,NFBX)+1) + 1
              IRZ = IBR(3,NFBSX)/(IBR(3,NFBX)+1) + 1
              NC = NC1X(IRX,IRZ)
              NBS = ICM(NC,2,NB)
              IF( IXP(NBS).NE.0 .AND. INBS(2,NB).EQ.0 ) THEN
                DVX = ABS(XE(1,N)-XE(7,NBS)) +
     &             ABS(YE(1,N)-YE(7,NBS)) + ABS(ZE(1,N)-ZE(7,NBS))
                IF( DVX.LT.1.D-9 ) THEN
...
!
            NBN = ICM(1,5,NB)
            IF( NBN.GT.0 ) THEN
              IF( NBN.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFBNX = INP(NBN-NFLD)
              ELSE
                NFBNX = NBN
              ENDIF
              IRX = IBR(1,NFBNX)/(IBR(1,NFBX)+1) + 1
              IRZ = IBR(3,NFBNX)/(IBR(3,NFBX)+1) + 1
              NC = NC2X(IRX,IRZ)
              NBN = ICM(NC,5,NB)
              IF( IXP(NBN).NE.0 .AND. INBS(2,NB).EQ.0 ) THEN
                DVX = ABS(XE(3,N)-XE(5,NBN)) +
     &             ABS(YE(3,N)-YE(5,NBN)) + ABS(ZE(3,N)-ZE(5,NBN))
                IF( DVX.LT.1.D-9 ) THEN
...
!
            NTN = ICM(1,5,NT)
            IF( NTN.GT.0 ) THEN
              IF( NTN.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFTNX = INP(NTN-NFLD)
              ELSE
                NFTNX = NTN
              ENDIF
              IRX = IBR(1,NFTNX)/(IBR(1,NFTX)+1) + 1
              IRZ = IBR(3,NFTNX)/(IBR(3,NFTX)+1) + 1
              NC = NC0X(IRX,IRZ)
              NTN = ICM(NC,5,NT)
...
!
            NTS = ICM(1,2,NT)
            IF( NTS.GT.0 ) THEN
              IF( NTS.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFTSX = INP(NTS-NFLD)
              ELSE
                NFTSX = NTS
              ENDIF
              IRX = IBR(1,NFTSX)/(IBR(1,NFTX)+1) + 1
              IRZ = IBR(3,NFTSX)/(IBR(3,NFTX)+1) + 1
              NC = NC0X(IRX,IRZ)
              NTS = ICM(NC,2,NT)
              IF( IXP(NTS).NE.0 .AND. INBS(2,NT).EQ.0 ) THEN
                DVX = ABS(XE(5,N)-XE(3,NTS)) +
     &             ABS(YE(5,N)-YE(3,NTS)) + ABS(ZE(5,N)-ZE(3,NTS))
                IF( DVX.LT.1.D-9 ) THEN
...
!
            NWB = ICM(1,1,NW)
            IF( NWB.GT.0 ) THEN
              IF( NWB.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFWBX = INP(NWB-NFLD)
              ELSE
                NFWBX = NWB
              ENDIF
              IRX = IBR(1,NFWBX)/(IBR(1,NFWX)+1) + 1
              IRY = IBR(2,NFWBX)/(IBR(2,NFWX)+1) + 1
              NC = NC3X(IRX,IRY)
              NWB = ICM(NC,1,NW)
              IF( IXP(NWB).NE.0 .AND. INBS(1,NW).EQ.0 ) THEN
                DVX = ABS(XE(1,N)-XE(6,NWB)) +
     &             ABS(YE(1,N)-YE(6,NWB)) + ABS(ZE(1,N)-ZE(6,NWB))
                IF( DVX.LT.1.D-9 ) THEN
...
!
            NEB = ICM(1,1,NE)
            IF( NEB.GT.0 ) THEN
              IF( NEB.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFEBX = INP(NEB-NFLD)
              ELSE
                NFEBX = NEB
              ENDIF
              IRX = IBR(1,NFEBX)/(IBR(1,NFEX)+1) + 1
              IRY = IBR(2,NFEBX)/(IBR(2,NFEX)+1) + 1
              NC = NC0X(IRX,IRY)
              NEB = ICM(NC,1,NE)
              IF( IXP(NEB).NE.0 .AND. INBS(1,NE).EQ.0 ) THEN
                DVX = ABS(XE(2,N)-XE(5,NEB)) +
     &             ABS(YE(2,N)-YE(5,NEB)) + ABS(ZE(2,N)-ZE(5,NEB))
                IF( DVX.LT.1.D-9 ) THEN
...
!
            NET = ICM(1,6,NE)
            IF( NET.GT.0 ) THEN
              IF( NET.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFETX = INP(NET-NFLD)
              ELSE
                NFETX = NET
              ENDIF
              IRX = IBR(1,NFETX)/(IBR(1,NFEX)+1) + 1
              IRY = IBR(2,NFETX)/(IBR(2,NFEX)+1) + 1
              NC = NC2X(IRX,IRY)
              NET = ICM(NC,6,NE)
...
!
            NWT = ICM(1,6,NW)
            IF( NWT.GT.0 ) THEN
              IF( NWT.GT.NFLD .AND. LBR.GT.0 ) THEN
                NFWTX = INP(NWT-NFLD)
              ELSE
                NFWTX = NWT
              ENDIF
              IRX = IBR(1,NFWTX)/(IBR(1,NFWX)+1) + 1
              IRY = IBR(2,NFWTX)/(IBR(2,NFWX)+1) + 1
              NC = NC1X(IRX,IRY)
              NWT = ICM(NC,6,NW)
...
!
      ELSEIF( JFLD.GT.1 ) THEN
        DO 250 N = 1,NFLD+NWN_LW
          IF( IXP(N).EQ.0 ) GOTO 250
          IF( N.GT.NFLD .AND. LBR.GT.0 .AND. LBR.GT.0 ) THEN
            NF = INP(N-NFLD)
          ELSE
            NF = N
          ENDIF
...
!
      ...
      REAL*8, DIMENSION(:), ALLOCATABLE :: XMX,YMX,ZMX,PORX,PERMX
      REAL*8 DISTX(8)
      CHARACTER*64 FMDUM
      CHARACTER*512 CHDUM
      LOGICAL FCHK
...
!
      ...
      ISUB_LOG = ISUB_LOG-1
      ALLOCATE( XMX(1:LVERT),STAT=ISTAT )
      IF( ISTAT.NE.0 ) THEN
        INDX = 3
        CHMSG = 'Allocation Error: XMX'
        CALL WRMSGS( INDX )
      ENDIF
      ALLOCATE( YMX(1:LVERT),STAT=ISTAT )
      IF( ISTAT.NE.0 ) THEN
        INDX = 3
        CHMSG = 'Allocation Error: YMX'
        CALL WRMSGS( INDX )
      ENDIF
      ALLOCATE( ZMX(1:LVERT),STAT=ISTAT )
      IF( ISTAT.NE.0 ) THEN
        INDX = 3
        CHMSG = 'Allocation Error: ZMX'
        CALL WRMSGS( INDX )
      ENDIF
      ALLOCATE( PORX(1:LVERT),STAT=ISTAT )
      IF( ISTAT.NE.0 ) THEN
        INDX = 3
        CHMSG = 'Allocation Error: PORX'
        CALL WRMSGS( INDX )
      ENDIF
      ALLOCATE( PERMX(1:LVERT),STAT=ISTAT )
      IF( ISTAT.NE.0 ) THEN
        INDX = 3
...
!
  ...
  444 CONTINUE
      CLOSE( UNIT=26 )
      DEALLOCATE( XMX,STAT=ISTAT )
      IF( ISTAT.NE.0 ) THEN
        INDX = 3
        CHMSG = 'Deallocation Error: XMX'
        CALL WRMSGS( INDX )
      ENDIF
      DEALLOCATE( YMX,STAT=ISTAT )
      IF( ISTAT.NE.0 ) THEN
        INDX = 3
        CHMSG = 'Deallocation Error: YMX'
        CALL WRMSGS( INDX )
      ENDIF
      DEALLOCATE( ZMX,STAT=ISTAT )
      IF( ISTAT.NE.0 ) THEN
        INDX = 3
        CHMSG = 'Deallocation Error: ZMX'
        CALL WRMSGS( INDX )
      ENDIF
...
!     rdgrid.F https://stash.pnnl.gov/scm/stomp/stomp.git V3.0
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!----------------------Subroutine--------------------------------------!
!
      SUBROUTINE DIST_PT( V0X,V1X,V2X,PX,DISTX )
!
!-------------------------Disclaimer-----------------------------------!
!
!     This material was prepared as an account of work sponsored by
...
!----------------------Description-------------------------------------!
!
!    Calculate the distance from a point PX to a 
!    triangle with vertices V0X, V1X, and V2X in three dimensions,
!    following the algorithm of Eberly, D. 2017. "Distance between 
...
!----------------------Type Declarations-------------------------------!
!
      REAL*8, DIMENSION(3) :: V0X,V1X,V2X
      REAL*8, DIMENSION(3) :: BX,E0X,E1X,PX
      REAL*8, DIMENSION(3) :: BMPX
!
!----------------------Executable Lines--------------------------------!
!
      ISUB_LOG = ISUB_LOG+1
      SUB_LOG(ISUB_LOG) = '/DIST_PT'
      DO M = 1,3
        BX(M) = V0X(M)
        E0X(M) = V1X(M)-V0X(M)
        E1X(M) = V2X(M)-V0X(M)
      ...
      DET = SAX*SCX - SBX*SBX
!
!---  Determine which region of the triangle contains (s,t)  ---
!
      IF( SSX+STX.LE.DET ) THEN
        IF( SSX.LE.0.D+0 ) THEN
          IF( STX.LE.0.D+0 ) THEN
            IRGX = 4
          ELSE
            IRGX = 3
          ENDIF
        ELSEIF( STX.LE.0.D+0 ) THEN
          IRGX = 5
        ELSE
          IRGX = 0 
        ENDIF
...
!---  Region 1  ---
!
      ELSEIF( IRGX.EQ.1 ) THEN
        VNX = (SCX+SEX) - (SBX+SDX)
        IF( VNX.LE.0.D+0 ) THEN
          SSX = 0.D+0
        ELSE
          VDX = SAX - 2.D+0*SBX + SCX
          IF( VNX.GE.VDX ) THEN
            SSX = 1.D+0
          ELSE
            SSX = VNX/VDX
...
!---  Region 2  ---
!
      ELSEIF( IRGX.EQ.2 ) THEN
        TMP0X = SBX + SDX
        TMP1X = SCX + SEX
        IF( TMP1X.GT.TMP0X ) THEN
          VNX = TMP1X - TMP0X
          VDX = SAX - 2.D+0*SBX + SCX
          IF( VNX.GE.VDX ) THEN
            SSX = 1.D+0
        ...
        ELSE
          SSX = 0.D+0
          IF( TMP1X.LE.0.D+0 ) THEN
            STX = 1.D+0
          ELSEIF( SEX.GE.0.D+0 ) THEN
            STX = 0.D+0
          ELSE
            STX = -SEX/SCX
          ENDIF
...
!---  Region 3  ---
!
      ELSEIF( IRGX.EQ.3 ) THEN
        SSX = 0.D+0
        IF( SEX.GE.0.D+0 ) THEN
          STX = 0.D+0
        ELSEIF( -SEX.GE.SCX ) THEN
          STX = 1.D+0
        ELSE
          STX = -SEX/SCX
        ENDIF
...
!---  Region 4  ---
!
      ELSEIF( IRGX.EQ.4 ) THEN
        IF( SDX.LT.0.D+0 ) THEN
          STX = 0.D+0
          IF( -SDX.GE.SAX ) THEN
            SSX = 1.D+0
          ELSE
...
!---  Region 5  ---
!
      ELSEIF( IRGX.EQ.5 ) THEN
        STX = 0.D+0
        IF( SDX.GE.0.D+0 ) THEN
          SSX = 0.D+0
        ELSEIF( -SDX.GE.SAX ) THEN
          SSX = 1.D+0
        ELSE
          SSX = -SDX/SAX
        ENDIF
...
!
      ELSEIF( IRGX.EQ.6 ) THEN
        ...
        ENDIF
      ENDIF
      DISTX = MAX( SAX*(SSX**2) + 2.D+0*SBX*SSX*STX + SCX*(STX**2) 
     &  + 2.D+0*SDX*SSX + 2.D+0*SEX*STX + SFX,0.D+0 )
      DISTX = SQRT( DISTX )
!
...
!
      ...
      ENDIF
!
!---  Closes AVS ASCII UCD File  ---
!
      CLOSE(UNIT=9)
      ISUB_LOG = ISUB_LOG-1
!
!---  End of RDAVSUCD group  ---
!
      RETURN
      END
      ...
      DO 710 N = 1,NFLD
        IF( IBR(4,N).GT.N ) THEN
          ...
          DO 700 NBRX = IBR(4,N),IBR(5,N)
            ...
            IZ(NBRX) = IZ(N)
            INP(NBRX-NFLD) = N
            IBR(1,NBRX) = IDF(IBRX,NX)
            IBR(2,NBRX) = JDF(IBRX,JBRX,NX)
            IBR(3,NBRX) = KDF(IBRX,JBRX,NX)
            IBR(4,NBRX) = NBRX
            IBR(5,NBRX) = NBRX
  700     CONTINUE
        ENDIF
...
!----------------------Description-------------------------------------!
!
!     Read hexahedral grid as an unstructured GSF.
!
!----------------------Authors-----------------------------------------!
!
!     Written by MD White, PNNL, 25 April 2019.
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!
      ...
      JFLDX = JFLD + 1
      KFLDX = KFLD + 1
      IF( NVERTX.NE.IFLDX*JFLDX*KFLDX ) THEN
        INDX = 7
        IMSG = NVERTX
...
!
      DO K = 1,KFLD
        DO J = 1,JFLD
          DO I = 1,IFLD
            N = ND(I,J,K)
            M = (J-1)*KFLDX*IFLDX + (I-1)*KFLDX + K
            XE(1,N) = VERTX(M)*VARX
            YE(1,N) = VERTY(M)*VARX
            ZE(1,N) = VERTZ(M)*VARX
            M = (J-1)*KFLDX*IFLDX + I*KFLDX + K
            XE(2,N) = VERTX(M)*VARX
            YE(2,N) = VERTY(M)*VARX
            ZE(2,N) = VERTZ(M)*VARX
...
!----------------------Subroutine--------------------------------------!
!
      SUBROUTINE RDMFDIS( XREFX,YREFX )
!
!-------------------------Disclaimer-----------------------------------!
!
!     This material was prepared as an account of work sponsored by
...
!
      ISUB_LOG = ISUB_LOG+1
      SUB_LOG(ISUB_LOG) = '/SURFN'
!
!---  Loop over unrefined nodes  ---
!
      DO N = 1,NFLD
        NB = ICM(5,1,N)
        NS = ICM(5,2,N)
        ...
        IF( INBS(6,N).GT.0 ) NQZ = INBS(6,N)
!
!---    Node to node connection on bottom side  ---
!
        IF( NB.NE.0 ) THEN
          DXX = XP(N)-XP(NB)
          DYX = YP(N)-YP(NB)
          ...
          SRFNZ(1,NPZ) = DXX/DISTX
          SRFNZ(2,NPZ) = DYX/DISTX
          SRFNZ(3,NPZ) = DZX/DISTX
!
!---    Node to surface connection on bottom side  ---
!
        ELSE
!
!---      Bottom surface centroid  ---
!
        ...
        ENDIF
!
!---    Node to node connection on south side  ---
!
        IF( NS.NE.0 ) THEN
          DXX = XP(N)-XP(NS)
          DYX = YP(N)-YP(NS)
          DZX = ZP(N)-ZP(NS)
          DISTX = SQRT(DXX**2 + DYX**2 + DZX**2)
          SRFNY(1,NPY) = DXX/DISTX
          SRFNY(2,NPY) = DYX/DISTX
          SRFNY(3,NPY) = DZX/DISTX
!
!---    Node to surface connection on south side  ---
!
        ELSE
!
!---      South surface centroid  ---
!
        ...
        ENDIF
!
!---    Node to node connection on west side  ---
!
        IF( NW.NE.0 ) THEN
          DXX = XP(N)-XP(NW)
          DYX = YP(N)-YP(NW)
          DZX = ZP(N)-ZP(NW)
          DISTX = SQRT(DXX**2 + DYX**2 + DZX**2)
          SRFNX(1,NPX) = DXX/DISTX
          SRFNX(2,NPX) = DYX/DISTX
          SRFNX(3,NPX) = DZX/DISTX
!
!---    Node to surface connection on west side  ---
!
        ELSE
!
!---      West surface centroid  ---
!
        ...
        ENDIF
!
!---    Node to node connection on east side  ---
!
        IF( NE.NE.0 ) THEN
          DXX = XP(NE)-XP(N)
          DYX = YP(NE)-YP(N)
          DZX = ZP(NE)-ZP(N)
          DISTX = SQRT(DXX**2 + DYX**2 + DZX**2)
          SRFNX(1,NQX) = DXX/DISTX
          SRFNX(2,NQX) = DYX/DISTX
          SRFNX(3,NQX) = DZX/DISTX
!
!---    Node to surface connection on east side  ---
!
        ELSE
!
!---      East surface centroid  ---
!
          ...
          ENDIF
!
!---    Node to node connection on north side  ---
!
        IF( NN.NE.0 ) THEN
          DXX = XP(NN)-XP(N)
          DYX = YP(NN)-YP(N)
          DZX = ZP(NN)-ZP(N)
          DISTX = SQRT(DXX**2 + DYX**2 + DZX**2)
          SRFNY(1,NQY) = DXX/DISTX
          SRFNY(2,NQY) = DYX/DISTX
          SRFNY(3,NQY) = DZX/DISTX
!
!---    Node to surface connection on west side  ---
!
        ELSE
...
!
        ...
        ENDIF
!
!---    Node to node connection on top side  ---
!
        IF( NT.NE.0 ) THEN
          DXX = XP(NT)-XP(N)
          DYX = YP(NT)-YP(N)
          DZX = ZP(NT)-ZP(N)
          DISTX = SQRT(DXX**2 + DYX**2 + DZX**2)
          SRFNZ(1,NQZ) = DXX/DISTX
          SRFNZ(2,NQZ) = DYX/DISTX
          SRFNZ(3,NQZ) = DZX/DISTX
!
!---    Node to surface connection on top side  ---
!
        ELSE
!
!---      Top surface centroid  ---
...
!----------------------Subroutine--------------------------------------!
!
      SUBROUTINE TRILINEAR( XDX,YDX,ZDX,CEX,CPX )
!
!-------------------------Disclaimer-----------------------------------!
!
!     This material was prepared as an account of work sponsored by
...
!----------------------Authors-----------------------------------------!
!
!     Written by MD White, PNNL, 28 February 2018
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!----------------------Type Declarations-------------------------------!
!
      REAL*8, DIMENSION(8) :: CEX
!
!----------------------Executable Lines--------------------------------!
!
      ISUB_LOG = ISUB_LOG+1
      SUB_LOG(ISUB_LOG) = '/TRILINEAR'
!
!---  Interpolate along x
!
      C00X = CEX(1)*(1.D+0-XDX) + CEX(2)*XDX
      C01X = CEX(5)*(1.D+0-XDX) + CEX(6)*XDX
      C10X = CEX(3)*(1.D+0-XDX) + CEX(4)*XDX
      C11X = CEX(7)*(1.D+0-XDX) + CEX(8)*XDX
!
!---  Interpolate along y
!
      C0X = C00X*(1.D+0-YDX) + C10X*YDX
      C1X = C01X*(1.D+0-YDX) + C11X*YDX
!
!---  Interpolate along z
!
      CPX = C0X*(1.D+0-ZDX) + C1X*ZDX
      ISUB_LOG = ISUB_LOG-1
!
!---  End of TRILINEAR group  ---
!
      RETURN
      END
...
!----------------------Authors-----------------------------------------!
!
!     Written by MD White, PNNL, 21 June 2022.
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE SOLTN
...
!
      VALUERSS=-1    
      OPEN(UNIT=100, FILE=PIDFNM, ACTION='READ')
      DO
...
120   CONTINUE
      CLOSE(100)
!
!---  End of VALUERSS group  ---
!
      RETURN
      END FUNCTION VALUERSS
