...
!     rdhydr.F https://stash.pnnl.gov/scm/stomp/stomp.git V3.0
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!
     ...
     &      'm^2: ',PERM(4,IROCK)
      ...
      ENDIF
!
!---  Permeability reduction factor models for STOMP-HYDT-KE  ---
!
      IF( IOM.EQ.39 ) THEN
!
!---    Check for hydrate-porosity model  ---
!
        ISTX = ISTART
        ICMX = ICOMMA
        CALL RDCHR(ISTART,ICOMMA,NCH,CHDUM,ADUM)
!
!---    Hydrate-porosity intrinsic permeability model   ---
!
        IF( INDEX(ADUM(1:),'hydrate').NE.0
     &    .AND. INDEX(ADUM(1:),'porosity').NE.0 ) THEN
          IF( IJK.GT.0 ) THEN
            DO N = 1,NFLD
              IPRF(N) = 4
            ENDDO
          ELSE
            IPRF(IROCK) = 4
          ENDIF
          WRITE(IWR,'(A)') 'Hydrate-Porosity Model'
          VARB = 'Hydrate m Coefficient'
          UNTS = 'null'
          INDX = 4
          LNDX = 9
          IF( IJK.GT.0 ) THEN
          ...
          ENDIF
          VARB = 'Initial Porosity Coefficient'
          UNTS = 'null'
          INDX = 6
          LNDX = 9
          IF( IJK.GT.0 ) THEN
          ...
          ENDIF
        ELSEIF( INDEX(ADUM(1:),'masuda').NE.0
     &    .AND. INDEX(ADUM(1:),'perm').NE.0 ) THEN
          IF( IJK.GT.0 ) THEN
            DO N = 1,NFLD
              IPRF(N) = 5
            ENDDO
          ELSE
            IPRF(IROCK) = 5
          ENDIF
          WRITE(IWR,'(A)') 'Masuda Effective Permeability Model'
          VARB = 'Exponent'
          UNTS = 'null'
          INDX = 4
          LNDX = 9
          ...
          ELSE
            ...
            WRITE(IWR,'(2X,2A,1PE11.4)')VARB(1:IVR),': ',PERM(6,IROCK)
          ENDIF
        ELSEIF( INDEX(ADUM(1:),'civan').NE.0 ) THEN
          IF( IJK.GT.0 ) THEN
            DO N = 1,NFLD
              IPRF(N) = 3
            ENDDO
          ELSE
            IPRF(IROCK) = 3
          ENDIF
          WRITE(IWR,'(A)') 'Civan Permeability Model'
        ...
        ELSE
          ...
          ELSE
            IPRF(IROCK) = 0
          ENDIF
          WRITE(IWR,'(A)') 'Original Porous Media Permeability Model'
        ENDIF
      ENDIF
!
!---  Check for permeability reduction factors for operational
!     modes other than STOMP-HYDT-KE  ---
!
      CALL CHKDPR( ISTART,ICOMMA,CHDUM,INDX )
      IF( INDX.EQ.1 .AND. IOM.NE.39 ) THEN
        IF( IJK.GT.0 ) THEN
          DO N = 1,NFLD
            IPRF(N) = 1
          ENDDO
