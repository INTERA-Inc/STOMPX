...
!  66   Aqueous Volumetric Fracture-Matrix Flux - UFML
!  67   Aqueous Volumetric Borehole Flux - UBBL
! 101 to 100+NSOLU
...
!     sfin.F https://stash.pnnl.gov/scm/stomp/stomp.git V3.0
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!
     ...
     &      // ' ---'
          ...
          WRITE(ISF(NSG),'(A,I6,/)') 'Surface Variables: ',NSF
!
...
!
     ...
     &          UNSF(1,NS)(1:NCH1),',',UNSF(2,NS)(1:NCH2),','
            ELSEIF( ISFT(NS).EQ.66 ) THEN
              WRITE(ISF(NSG),'(6A)') 'Aqueous Volumetric ' // 
...
!
     ...
     &        I.LE.(100+NSOLU+NEQC+NEQK) ) THEN
            ...
            J = ABS( ISFD(NS) )
            IF( ISFD(NS).EQ.4 .OR. ISFD(NS).EQ.5 .OR. ISFD(NS).EQ.6 )
     &        J = 1
...
!
        ELSEIF( ISFDX.EQ.5 ) THEN
          NFX = ISFCX(1)
          DO NX = ISFCX(2),ISFCX(3)
            NTX = IP_FRC(1,NFX) - 1 + NX
            DO NCX = IPN_FRC(1,NTX),IPN_FRC(2,NTX)
              IF( ISFT(NS).EQ.66 ) THEN
                SFX = UFML(NCX)
              ENDIF
              SF(1,NS) = SF(1,NS)+SFX
            ENDDO
          ENDDO
!
!---    Borehole-borehole surface  ---
!
        ELSEIF( ISFDX.EQ.6 ) THEN
          NBHX = ISFCX(1)
          NBN1X = ID_BH(3,NBHX) - 1 + ISFCX(2)
          NBN2X = ID_BH(3,NBHX) - 1 + ISFCX(3)
          DO NCX = IPB_BH(1,NBN1X),IPB_BH(2,NBN1X)
            IF( NCX.EQ.0 ) CYCLE
            IF( NBN2X.EQ.IBCM_BH(NCX) ) THEN
              IF( ISFT(NS).EQ.67 ) THEN
                DBB1X = DBBM_BH(NCX)
...
!
          IF( ISFF(NS).EQ.NSG ) THEN
            NCTX(ISFF(NS)) = NCTX(ISFF(NS)) + 1
            IF( NS.EQ.NSF .OR. NCTX(ISFF(NS)).EQ.ISFGP(NSG) ) THEN
