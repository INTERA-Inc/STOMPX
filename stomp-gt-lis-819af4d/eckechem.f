...
!
              ...
              DO NSL = 1,NSOLU
                C_FRC(NTX,NSL) = CO_FRC(NTX,NSL)
              ENDDO

              DO NSP = 1,NSPR
                SP_C_FRC(NTX,NSP) = SP_CO_FRC(NTX,NSP)
              ENDDO
...
!
            DO NBN = ID_BH(3,NBH),ID_BH(4,NBH)
              ...
              DO NSL = 1,NSOLU
                C_BH(NBN,NSL) = CO_BH(NBN,NSL)
              ENDDO

              DO NSP = 1,NSPR
                SP_C_BH(NBN,NSP) = SP_CO_BH(NBN,NSP)
              ENDDO
...
!
      ...
      REAL*8 ACTCX(LSPL)
      REAL*8 RSDX(LSPR)
      REAL*8, DIMENSION(:), ALLOCATABLE :: RSDMAX
      INTEGER IJM(LSPR)
...
!
      ...
      ALLOCATE( RSDMAX(1:NCX),STAT=ISTAT )
      IF( ISTAT.NE.0 ) THEN
        INDX = 3
        CHMSG = 'Allocation Error: RSDMAX'
        CALL WRMSGS( INDX )
      ENDIF
      DO NC = 1,NCX
        RSDMAX(NC) = 0.D+0
...
!
!---      Check for disappearing equilibrium or conservation species  --
!
          IF( NSP.LE.(NEQE+NEQC) ) THEN
            VARX = SP_CX(NSP)+BJM(IEQ_S(NSP))
            IF( VARX.LT.0.D+0 .AND. ABS(VARX).GT.1.D+3*SP_CX(NSP) ) THEN
              RSDX(NSP) = 0.D+0
...
!
      ...
      ENDDO
      IF( ALLOCATED(RSDMAX) ) THEN
        DEALLOCATE( RSDMAX,STAT=ISTAT )
        IF( ISTAT.NE.0 ) THEN
          INDX = 3
          CHMSG = 'Allocation Error: RSDMAX'
          CALL WRMSGS( INDX )
...
!
     ...
     &          IOM.EQ.43 ) SMDG(NSL) = SP_MDG
              IF( IEQO.GT.0 .OR. IOM.EQ.43 ) SMDN(NSL) = SP_MDN
              C(N,NSL) = 0.D+0
...
!                PRINT '(A,I9)','Node = ',N_DB
!                PRINT '(A)','Execution Error: GAUSSJ: Pivot Error'
                IERX = -1
                RETURN
              ENDIF
            ENDDO
...
!
!     Add immobile species concentrations into component species
!     concentrations for boreholes.
!
!     C_BH(NBN,NSL) fracture component species concentration 
!       (kmol/m^3 node)
!     SP_C_BH(NBN,NSP) fracture species concentration (kmol/m^3 node)
!
!----------------------Authors-----------------------------------------!
!
!     Written by Mark White, PNNL, October 18, 2019.
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!
        IF( ISPLK(4).EQ.NSL ) THEN
          COX = 1.D+3*XLA_BH(1,NBN)*RHOL_BH(1,NBN)*SL_BH(1,NBN)*
     &      PORD_BH(1,NBN)/WTMA
          CX = 1.D+3*XLA_BH(2,NBN)*RHOL_BH(2,NBN)*SL_BH(2,NBN)*
     &      PORD_BH(2,NBN)/WTMA
          C_BH(NBN,NSL) = COX + (CX-COX)*DT/DT_RST
          GOTO 110
        ENDIF
!
!---    Linked aqueous CO2   ---
...
!
        ...
        DO IGC = 1,NGC+ISHIFTX
          IF( ISPLK(14+NSPLK+IGC).EQ.NSL ) THEN
            COX = 1.D+3*XLC(IGC,1,N)*RHOL_BH(1,NBN)*SL_BH(1,NBN)*
     &        PORD_BH(1,NBN)/GCPP(1,IGC)
            CX = 1.D+3*XLC(IGC,2,N)*RHOL_BH(2,NBN)*SL_BH(2,NBN)*
     &        PORD_BH(2,NBN)/GCPP(1,IGC)
            C_BH(NBN,NSL) = COX + (CX-COX)*DT/DT_RST
              GOTO 110
          ENDIF
        END DO
...
!
        ...
        DO IGC = 1,NGC+ISHIFTX
          IF( ISPLK(14+NSPLK+NGC+ISHIFTX+IGC).EQ.NSL ) THEN
            COX = 1.D+3*XGC(IGC,1,N)*RHOG_BH(1,NBN)*SG_BH(1,NBN)*
     &        PORD_BH(1,NBN)/GCPP(1,IGC)
            CX = 1.D+3*XGC(IGC,2,N)*RHOG_BH(2,NBN)*SG_BH(2,NBN)*
     &        PORD_BH(2,NBN)/GCPP(1,IGC)
            C_BH(NBN,NSL) = COX + (CX-COX)*DT/DT_RST
            GOTO 110
          ENDIF
...
!
          IF( NSP.GT.NSPL .AND. NSP.LE.(NSPL+NSPS+NSPE) ) THEN
            IF( ABS(SP_C_BH(NBN,NSP)).LT.CMIN ) THEN
              SP_CX = 0.D+0
            ELSE
              SP_CX = SP_C_BH(NBN,NSP)
            ENDIF
            C_BH(NBN,NSL) = C_BH(NBN,NSL) + EQ_C(M,NEQ)*SP_CX
          ENDIF
        ENDDO
  110   CONTINUE
!
...
!
!     Add immobile species concentrations into component species
!     concentrations for fractures.
!
!     C_FRC(NTX,NSL) fracture component species concentration 
!       (kmol/m^3 node)
!     SP_C_FRC(NTX,NSP) fracture species concentration (kmol/m^3 node)
!
!----------------------Authors-----------------------------------------!
!
!     Written by Mark White, PNNL, October 18, 2019.
...
!
        IF( ISPLK(4).EQ.NSL ) THEN
          COX = 1.D+3*XLA_FRC(1,NTX)*RHOL_FRC(1,NTX)*SL_FRC(1,NTX)*
     &      PORD_FRC(2,NTX)/WTMA
          CX = 1.D+3*XLA_FRC(2,NTX)*RHOL_FRC(2,NTX)*SL_FRC(2,NTX)*
     &      PORD_FRC(2,NTX)/WTMA
          C_FRC(NTX,NSL) = COX + (CX-COX)*DT/DT_RST
          GOTO 110
        ENDIF
!
!---    Linked aqueous CO2   ---
...
!
        ...
        DO IGC = 1,NGC+ISHIFTX
          IF( ISPLK(14+NSPLK+IGC).EQ.NSL ) THEN
            COX = 1.D+3*XLC(IGC,1,N)*RHOL_FRC(1,NTX)*SL_FRC(1,NTX)*
     &        PORD_FRC(2,NTX)/GCPP(1,IGC)
            CX = 1.D+3*XLC(IGC,2,N)*RHOL_FRC(2,NTX)*SL_FRC(2,NTX)*
     &        PORD_FRC(2,NTX)/GCPP(1,IGC)
            C_FRC(NTX,NSL) = COX + (CX-COX)*DT/DT_RST
              GOTO 110
          ENDIF
...
!
        ...
        DO IGC = 1,NGC+ISHIFTX
          IF( ISPLK(14+NSPLK+NGC+ISHIFTX+IGC).EQ.NSL ) THEN
            COX = 1.D+3*XGC(IGC,1,N)*RHOG_FRC(1,NTX)*SG_FRC(1,NTX)*
     &        PORD_FRC(2,NTX)/GCPP(1,IGC)
            CX = 1.D+3*XGC(IGC,2,N)*RHOG_FRC(2,NTX)*SG_FRC(2,NTX)*
     &        PORD_FRC(2,NTX)/GCPP(1,IGC)
            C_FRC(NTX,NSL) = COX + (CX-COX)*DT/DT_RST
            GOTO 110
          ENDIF
...
!
          IF( NSP.GT.NSPL .AND. NSP.LE.(NSPL+NSPS+NSPE) ) THEN
            IF( ABS(SP_C_FRC(NTX,NSP)).LT.CMIN ) THEN
              SP_CX = 0.D+0
            ELSE
              SP_CX = SP_C_FRC(NTX,NSP)
            ENDIF
            C_FRC(NTX,NSL) = C_FRC(NTX,NSL) + EQ_C(M,NEQ)*SP_CX
          ENDIF
        ENDDO
  110   CONTINUE
!
...
!
          IF( NSP.GT.NSPL .AND. NSP.LE.(NSPL+NSPS+NSPE) ) THEN
            ...
            ELSE
              SP_CX = SP_C(N,NSP)
            ENDIF
            C_BH(NBN,NSL) = C_BH(NBN,NSL) + EQ_K(M,NEQ)*SP_CX
          ENDIF
        ENDDO
  110   CONTINUE
!
...
!
          IF( NSP.GT.NSPL .AND. NSP.LE.(NSPL+NSPS+NSPE) ) THEN
            ...
            ELSE
              SP_CX = SP_C(N,NSP)
            ENDIF
            C_FRC(NTX,NSL) = C_FRC(NTX,NSL) + EQ_K(M,NEQ)*SP_CX
          ENDIF
        ENDDO
  110   CONTINUE
!
...
!
     ...
     &      NSP.LE.(NSPL+NSPS+NSPE+NSPG+NSPN) .AND. 
     &      (IEQO.GT.0.OR.IOM.EQ.43) ) THEN
...
!----------------------Description-------------------------------------!
!
!     Mobile conservation component fractions for boreholes.
!
!     YSPLX aqueous fraction of component species NEQ at borehole 
!       node NBN
!     YSPGX gas fraction of component species NEQ at borehole 
!       node NBN
!     YSPNX NAPL fraction of component species NEQ at borehole 
!       node NBN
!     C_BH(NBN,NSL) borehole component species concentration 
!       (kmol/m^3 node)
!     SP_C_BH(NBN,NSP) borehole species concentration (kmol/m^3 node)
!
!----------------------Authors-----------------------------------------!
!
!     Written by Mark White, PNNL, October 18, 2019.
...
!
      DO NBN = ID_BH(3,NBH),ID_BH(4,NBH)
        ...
        YSPNZ = 0.D+0
        NSL = NSOLU + NEQ
        IF( ICT_BH(NBN,NSL).NE.0 .AND. NSTEP.EQ.0 ) CYCLE
        C_BH(NBN,NSL) = 0.D+0
!
!---    Loop over conservation-component species  ---
!
...
!
     ...
     &      (IEQO.GT.0.OR.IOM.EQ.43) ) THEN
            YSPNX = YSPNX + EQ_C(M,NEQ)*SP_CX
            YSPNZ = YSPNZ + EQ_C(M,NEQ)
            C_BH(NBN,NSL) = C(NTX,NSL) + EQ_C(M,NEQ)*SP_CX
          ENDIF
        ENDDO
!
!---    Update old time step conservation-component species ---
...
!---      Zero species concentration  ---
!
          ELSEIF( ABS(C_BH(NBN,NSL))/EPSL.LT.EPSL ) THEN
            YSPLX = YSPLZ/YSPZ
!
!---      Non-zero species concentration  ---
!
          ELSE
            YSPLX = YSPLX/C_BH(NBN,NSL)
          ENDIF
          YL_BH(NBN,NSL) = YSPLX
!
!---      pH link ---
...
!----------------------Description-------------------------------------!
!
!     Mobile conservation component fractions for fractures.
!
!     YSPLX aqueous fraction of component species NEQ at fracture 
!       triangle NTX
!     YSPGX gas fraction of component species NEQ at fracture 
!       triangle NTX
!     YSPNX NAPL fraction of component species NEQ at fracture 
!       triangle NTX
!     C_FRC(NTX,NSL) fracture component species concentration 
!       (kmol/m^3 node)
!     SP_C_FRC(NTX,NSP) fracture species concentration (kmol/m^3 node)
...
!
        ...
        YSPNZ = 0.D+0
        NSL = NSOLU + NEQ
        IF( ICT_FRC(NTX,NSL).NE.0 .AND. NSTEP.EQ.0 ) CYCLE
        C_FRC(NTX,NSL) = 0.D+0
!
!---    Loop over conservation-component species  ---
!
...
!
     ...
     &      (IEQO.GT.0.OR.IOM.EQ.43) ) THEN
            YSPNX = YSPNX + EQ_C(M,NEQ)*SP_CX
            YSPNZ = YSPNZ + EQ_C(M,NEQ)
            C_FRC(NTX,NSL) = C(NTX,NSL) + EQ_C(M,NEQ)*SP_CX
          ENDIF
        ENDDO
!
!---    Update old time step conservation-component species ---
...
!---      Zero species concentration  ---
!
          ELSEIF( ABS(C_FRC(NTX,NSL))/EPSL.LT.EPSL ) THEN
            YSPLX = YSPLZ/YSPZ
!
!---      Non-zero species concentration  ---
!
          ELSE
            YSPLX = YSPLX/C_FRC(NTX,NSL)
          ENDIF
          YL_FRC(NTX,NSL) = YSPLX
!
!---      pH link ---
!
          IF( ISPLK(1).EQ.NSL ) THEN
            YL_FRC(NTX,NSL) = 1.D+0
!
!---      Air link ---
!
          ELSEIF( ISPLK(4).EQ.NSL ) THEN
...
!
          ELSE
            YSPGX = YSPGX/C_FRC(NTX,NSL)
          ENDIF
          YG_FRC(NTX,NSL) = YSPGX
!
!---      Component link ---
!
          ISHIFTX=0
...
!
          ELSE
            YSPNX = YSPNX/C_FRC(NTX,NSL)
          ENDIF
          YN_FRC(NTX,NSL) = YSPNX
        ENDIF
      ENDDO
      ENDDO
!
...
5080  FORMAT(/64X,20A20)
      ...
      NT = 0
!     
!---  Modification made in code cleanup on 09/20/2018  ---
!      DO i=1,ncc
!        DO j=i+1,ncc
      DO I = 1,NCC_PZ
        DO J = I+1,NCC_PZ
...
!
     ...
     &      'Parameter LSPK'
        ...
        IF( IRCKT(NRC).EQ.2 .AND. NSPRX.NE.3 ) THEN
          ...
          CALL WRMSGS( INDX )
        ENDIF
        IF( IRCKT(NRC).EQ.17 .AND. NSPRX.NE.3 ) THEN
          INDX = 4
          CHMSG = 'Schroth-Monod Kinetics: Number of Reactants != 3'
          CALL WRMSGS( INDX )
        ENDIF
        ...
        IF( IRCKT(NRC).EQ.36 .AND. NSPRX.NE.2 ) THEN
          ...
          CALL WRMSGS( INDX )
        ENDIF
        IF( IRCKT(NRC).EQ.37 .AND. NSPRX.NE.2 .AND. NSPRX.NE.3 ) THEN
          INDX = 4
          CHMSG = 'Dual-Monod-Inhibitor Kinetics:  ' // 
     &      'Number of Reactants != 2 and != 3'
          CALL WRMSGS( INDX )
        ENDIF
...
!
     ...
     &        'Kinetic Reaction > Parameter LSPK'
          ...
          IF( IRCKT(NRC).EQ.2 .AND. NSPPX.NE.0 ) THEN
            ...
            CALL WRMSGS( INDX )
          ENDIF
          IF( IRCKT(NRC).EQ.17 .AND. NSPPX.NE.0 ) THEN
            INDX = 4
            CHMSG = 'Schroth-Monod Kinetics: Number of Products != 0'
            CALL WRMSGS( INDX )
          ENDIF
     ...
     &        'Number of Products != 1'
          ...
          IF( IRCKT(NRC).EQ.36 .AND. NSPPX.NE.0 ) THEN
            ...
            CALL WRMSGS( INDX )
          ENDIF
          IF( IRCKT(NRC).EQ.37 .AND. NSPPX.NE.2 .AND. NSPPX.NE.3  ) THEN
            INDX = 4
            CHMSG = 'Dual-Monod-Inhibitor Kinetics: ' // 
     &        'Number of Products ! = 2 and ! = 3'
            CALL WRMSGS( INDX )
          ENDIF
...
!
        ELSEIF( IRCKT(NRC).EQ.17 ) THEN
!
!---      Read half-saturation constant for donor  ---
!
          VARB = 'Schroth-Monod Kin. Reac.: ' //
     &      'Half-Sat. Const. for Donor'
          UNTS = 'mol/kg'
          IUNMOL = 1
          IUNKG = -1
...
!         RC_K(JCX,NRC) = 0.D+0
     ...
     &        ', 1/s)'
          ENDIF
!
!---      Read half-saturation constant for limiter  ---
!
          VARB = 'Schroth-Monod Kin. Reac.: ' //
     &      'Half-Sat. Const. for Limiters'
          UNTS = 'mol/kg'
          IUNMOL = 1
          IUNKG = -1
          IDFLT = 1
...
!
     ...
     &      ', mol/kg)'
          ENDIF
!
!---      Number of reaction limiter species  ---
!
          ICX = ICX+1
          VARB = 'Number of Limiter Species'
          CALL RDINT( ISTART,ICOMMA,CHDUM,IRC_K(ICX,NRC) )
          IF( IRC_K(ICX,NRC).GT.0 ) THEN
            WRITE (IWR,'(/,A)') ' Limiter Species List '
          ENDIF
!
!---      Loop over number of reaction limiter species  ---
!
          DO J = 1,IRC_K(ICX,NRC)
            VARB = 'Limiter Species Name: '
            ICX = ICX+1
!
!---        Allow for returns in input lines  ---
!
...
!
            ...
            IF( IFOUND.EQ.0 ) THEN
              INDX = 4
              CHMSG = 'Unrecognized Limiter Species Name: ' //
     &          ADUM(1:NCH)
              CALL WRMSGS( INDX )
            ENDIF
            WRITE (IWR,'(2X,2A)') ' Limiter Species Name: ',ADUM
          ENDDO
!
...
!         RC_K(JCX,NRC) = 0.D+0
     ...
     &        ', 1/s)'
          ENDIF
!
!---    Dual-Monod-Inhibitor type reactions (aqueous reactions)  ---
!
        ELSEIF( IRCKT(NRC).EQ.37 ) THEN
!
!---      Read reaction rate constant  ---
!
          VARB = 'Dual-Monod-Inhibitor Kinetic Reaction Rate Constant'
          UNTS = 'mol/m^3 s'
          IUNMOL = 1
          IUNM = -3
          IUNS = -1
...
!         RC_K(JCX,NRC) = 0.D+0
          IF( INDEX(CHDUM(ISTART:ISTART+5),'file') /= 0 ) THEN
            ...
            DO J = 1,LRC
!            
!---        Convert from kmol/m^3 s to mol/m^3 s ---
!          
              RC_K(JCX,J,NRC) = 1.D+3*RC_K(JCX,J,NRC)
            ENDDO
          ELSE
...
!---      Read half-saturation constant for donor  ---
!
          VARB = 'Dual-Monod-Inhibitor Kinetic Reaction: ' //
     &      'Half-Sat. Const. for Donor'
          UNTS = 'mol/m^3'
          IUNMOL = 1
          IUNM = -3
...
!
     ...
     &      ', mol/m^3)'
          ENDIF
!
!---      Read half-saturation constant for inhibitor  ---
!
          VARB = 'Dual-Monod-Inhibitor Kinetic Reaction: ' //
     &      'Half-Sat. Const. for Inhibitor'
          UNTS = 'mol/m^3'
          IUNMOL = 1
          IUNM = -3
          IDFLT = 1
...
!
        DO 270 NX = 1,NFLD
          IF( IJK.GT.0 ) THEN
              ...
              IF( ISLC(86).EQ.1 ) THEN
                VARX = RS_SX(2,NX)*(1.D+0-POR(2,NX))
              ELSE
                VARX = RS_SX(2,NX)
              ENDIF
              RS_S(2,NSP,NX) = VARX
              ...
              POR0(3,NX) = POR0(3,NX) + VARX
          ELSE
            IF( IROCK.EQ.IZ(NX) ) THEN
              RS_S(1,NSP,NX) = RS_SX(1,IROCK)
              IF( ISLC(86).EQ.1 ) THEN
                VARX = RS_SX(2,IROCK)*(1.D+0-POR(2,IROCK))
              ELSE
                VARX = RS_SX(2,IROCK)
              ENDIF
              RS_S(2,NSP,NX) = VARX
...
!
!     Load old reactive-species concentrations and
!     component-species concentrations for boreholes.
!
!----------------------Authors-----------------------------------------!
!
!     Written by Mark White, PNNL, October 18, 2019.
...
!
!     Load old reactive-species concentrations and
!     component-species concentrations for fractures.
!
!----------------------Authors-----------------------------------------!
!
!     Written by Mark White, PNNL, October 18, 2019.
...
!----------------------Description-------------------------------------!
!
!     Write ptzrcoef.bin file
!
!----------------------Authors-----------------------------------------!
!
!     Written by MD White, PNNL, 2 February 2022.
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!----------------------Executable Lines--------------------------------!
!
      IF( IACTV.NE.2 ) RETURN
      ISUB_LOG = ISUB_LOG+1
      SUB_LOG(ISUB_LOG) = '/WRITE_PTZR'
!
!---  Read Pitzer parameters  ---
!
      CALL RDPTZR
!
!---  Open a new ptzrcoef.bin file  --
!
      OPEN( UNIT=20,FILE='ptzrcoef.bin',STATUS='UNKNOWN',
     &  FORM='UNFORMATTED' )
      CLOSE(UNIT=20,STATUS='DELETE')
      OPEN(UNIT=20, FILE='ptzrcoef.bin', STATUS='NEW', 
     &  FORM='UNFORMATTED')
!
!---  Write Pitzer coefficient integers  ---
!
      WRITE(20) NCC_PZ,NA_PZ,NNN_PZ
!
!---  Write Pitzer coefficient arrays  ---
!
      WRITE(20) (JPA(L),L=1,LANI)
      WRITE(20) (JPC(L),L=1,LCAT)
      WRITE(20) (JPN(L),L=1,LNEU)
      WRITE(20) ((B0(K,L),K=1,LCAT),L=1,LANI)
      WRITE(20) ((B1(K,L),K=1,LCAT),L=1,LANI)
      WRITE(20) ((B2(K,L),K=1,LCAT),L=1,LANI)
      WRITE(20) ((CMXX(K,L),K=1,LCAT),L=1,LANI)
      WRITE(20) (TCC(L),L=1,LNCF)
      WRITE(20) (TAA(L),L=1,LNAF)
      WRITE(20) ((PSIC(K,L),K=1,LNCF),L=1,LANI)
      WRITE(20) ((PSIA(K,L),K=1,LNAF),L=1,LCAT)
      WRITE(20) ((ALAMB(K,L),K=1,LNEU),L=1,LANI)
      WRITE(20) ((CLAMB(K,L),K=1,LNEU),L=1,LCAT)
      WRITE(20) ((ELAMB(K,L),K=1,LNEU),L=1,LNEU)
      WRITE(20) ((HOLAMB(K,L),K=1,LNNF),L=1,LANI)
      WRITE(20) ((BPPR(K,L),K=1,LCAT),L=1,LANI)
      WRITE(20) ((BPHI(K,L),K=1,LCAT),L=1,LANI)
      WRITE(20) ((BPR(K,L),K=1,LCAT),L=1,LANI)
      WRITE(20) ((BMMX(K,L),K=1,LCAT),L=1,LANI)
      WRITE(20) (((ATB0(K,L,M),K=1,LCAT),L=1,LANI),M=1,8)
      WRITE(20) (((ATB1(K,L,M),K=1,LCAT),L=1,LANI),M=1,8)
      WRITE(20) (((ATB2(K,L,M),K=1,LCAT),L=1,LANI),M=1,8)
      WRITE(20) (((ATCMX(K,L,M),K=1,LCAT),L=1,LANI),M=1,8)
      WRITE(20) (((ATNLAM(K,L,M),K=1,LNEU),L=1,LNEU),M=1,6)
      WRITE(20) (((ATCLAM(K,L,M),K=1,LNEU),L=1,LCAT),M=1,6)
      WRITE(20) (((ATALAM(K,L,M),K=1,LNEU),L=1,LANI),M=1,6)
      WRITE(20) (((ATHLAM(K,L,M),K=1,LNNF),L=1,LANI),M=1,6)
      WRITE(20) ((ATTC(K,L),K=1,LNCF),L=1,6)
      WRITE(20) (((ATPC(K,L,M),K=1,LNCF),L=1,LANI),M=1,6)
      WRITE(20) ((ATTA(K,L),K=1,LNAF),L=1,6)
      WRITE(20) (((ATPA(K,L,M),K=1,LNAF),L=1,LCAT),M=1,6)
      WRITE(20) (CTCPH(L),L=1,LNCF)
      WRITE(20) (CTC(L),L=1,LNCF)
      WRITE(20) (CTCPR(L),L=1,LNCF)
      WRITE(20) (CTCPPR(L),L=1,LNCF)
      WRITE(20) (CTAPH(L),L=1,LNAF)
      WRITE(20) (CTA(L),L=1,LNAF)
      WRITE(20) (CTAPR(L),L=1,LNAF)
      WRITE(20) (CTAPPR(L),L=1,LNAF)
      WRITE(20) (ETH(L),L=1,LMCG*LMCG)
      WRITE(20) (ETHP(L),L=1,LMCG*LMCG)
      WRITE(20) (ETHP2(L),L=1,LMCG*LMCG)
      WRITE(20) (IDD_PZ(L),L=1,LSPL)
!
!---  Close the ptzrcoef.bin file  ---
!
      CLOSE(20)
!
!---  Reset subroutine string sequence  ---
...
!
      ISUB_LOG = ISUB_LOG+1
      SUB_LOG(ISUB_LOG) = '/WRITE_REACT'
!
!---  Open a new 1 .bin file  --
!
      OPEN( UNIT=20,FILE='ecke.bin',STATUS='UNKNOWN',
     &  FORM='UNFORMATTED' )
      CLOSE(UNIT=20,STATUS='DELETE')
      OPEN(UNIT=20, FILE='ecke.bin', STATUS='NEW', 
     &  FORM='UNFORMATTED')
!
!---  Write activity model index, number of reactive equations,
!     and species  ---
!
      WRITE(20) IACTEX,IACTV,ISP_IEDL,NEQE,NEQC,NEQK,NESITE,
     &  NRCE,NRCK,NRTSI,NSPC,NSPE,NSPG,NSPK,NSPL,NSPLK,NSPN,
     &  NSPR,NSPS
!
!---  Write reactive transport parameters, indices, and variables  ---
!
      WRITE(20) ((IEQ_E(L,M),L=1,LSEE+2),M=1,LEQE)
      WRITE(20) ((IEQ_C(L,M),L=1,LSEC+1),M=1,LEQC)
      WRITE(20) ((IEQ_K(L,M),L=1,LSEK+LREK+2),M=1,LEQK)
      WRITE(20) (IEQ_S(L),L=1,LSPR)
      WRITE(20) (ISP_E(L),L=1,LSPE)
      WRITE(20) (IEL_LK(L),L=1,LSPE)
      WRITE(20) (ISP_MN(L),L=1,LSPR)
      WRITE(20) (ISP_S(L),L=1,LEQE+LEQC+LEQK)
      WRITE(20) (IMMB(L),L=1,LSOLU+LSPT)
      WRITE(20) ((IRC_K(L,M),L=1,LSPK+3),M=1,LRCK)
      WRITE(20) (IRCKN(L),L=1,LSPK+11)
      WRITE(20) (IRCKT(L),L=1,LRCK)
      WRITE(20) (ISPLK(L),L=1,2*LNGC+LSPR+14)
      DO J = 1,LSPS
        WRITE(20) ((ISP_OW(J,ND_MPI(K,L)),K=1,NC_MPI(L)),L=1,NP_MPI)
      ENDDO
      DO M = 1,LSPR
        WRITE(20) ((IC_SP(ND_MPI(K,L),M),K=1,NC_MPI(L)),L=1,NP_MPI)
      ENDDO
      WRITE(20) ACTVC,CMIN,SP_MDG,SP_MDL
      WRITE(20) (SP_SDCL(L),L=1,3)
      WRITE(20) (CFMX(L),L=1,LMC)
      WRITE(20) (CHARG(L),L=1,LSPR)
      WRITE(20) ((EQ_C(L,M),L=1,LSEC),M=1,LEQC)
      WRITE(20) ((EQ_E(L,M),L=1,LSEE),M=1,LEQE)
      WRITE(20) ((EQ_K(L,M),L=1,LSEK+LREK),M=1,LEQK)
      WRITE(20) ((RC_E(L,M),L=1,5),M=1,LRCE)
      WRITE(20) (((RC_K(K,L,M),K=1,LSPK+11),L=1,LCKN),M=1,LRCK)
      WRITE(20) ((SP_L(L,M),L=1,3),M=1,LSPL)
      WRITE(20) ((SP_S(L,M),L=1,2),M=1,LSPS)
      DO I = 1,3
        DO J = 1,LSPS
          WRITE(20) ((RS_S(I,J,ND_MPI(K,L)),K=1,NC_MPI(L)),L=1,NP_MPI)
        ENDDO
      ENDDO
      DO M = 1,LEQC+LEQK
        WRITE(20) ((YSPG(ND_MPI(K,L),M),K=1,NC_MPI(L)),L=1,NP_MPI)
      ENDDO
      DO M = 1,LEQC+LEQK
        WRITE(20) ((YSPL(ND_MPI(K,L),M),K=1,NC_MPI(L)),L=1,NP_MPI)
      ENDDO
      DO M = 1,LSPR
        WRITE(20) ((SP_C(ND_MPI(K,L),M),K=1,NC_MPI(L)),L=1,NP_MPI)
      ENDDO
      WRITE(20) (RCNME(L),L=1,LRCE)
      WRITE(20) (RCNMK(L),L=1,LRCK)
      WRITE(20) (SPNMC(L),L=1,LEQC)
      WRITE(20) (SPNMK(L),L=1,LEQK)
      WRITE(20) (SPNMG(L),L=1,LSPG)
      WRITE(20) (SPNML(L),L=1,LSPL)
      WRITE(20) (SPNMS(L),L=1,LSPS)
      WRITE(20) (SPNME(L),L=1,LSPE)
!
!---  Close the 1 .bin file  ---
!
      CLOSE(20)
!
!---  Reset subroutine string sequence  ---
