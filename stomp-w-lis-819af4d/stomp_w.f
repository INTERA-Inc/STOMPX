...
!
        ...
        CALL WRITE_BIN_W
        WRITE(IWR,'(A)') 'NOTE: Preprocessing for STOMPX-W'
        WRITE(ISC,'(A)') 'NOTE: Preprocessing for STOMPX-W'
!
!---    End parallel pre-processing unless output of the flow or
!       transport linear system is requested  ---
!
        IF( ISLC(34).EQ.0 ) THEN
          WRITE(IWR,'(/,A,/)') '---  End of STOMPX-W Preprocessing  ---'
          WRITE(ISC,'(/,A,/)') '---  End of STOMPX-W Preprocessing  ---'
          STOP
        ENDIF
      ENDIF
!
...
!
        ...
        ENDIF
!
!---    Gradient for x-y-z hydraulic gradient
...
!
          ELSEIF( ABS(IBCT(IEQW,NB)).EQ.11 ) THEN
            ...
            ELSEIF( IBCT(IEQW,NB).EQ.11 ) THEN
              IF( ISLC(67).EQ.1 ) THEN
                PLX = BCX(2)
                CALL FNHGBLX( XSBC(MB),YSBC(MB),ZSBC(MB),XPBC(NB),
...
!----------------------Type Declarations-------------------------------!
!
      CHARACTER*64 ADUM,BDUM(LUK+LSOLU+1),FDUM
      CHARACTER*64 UNTS
      CHARACTER*24 CHLB(3)
...
!     stomp_w.F 1344 2020-07-28 22:36:16Z d3c002 https://stash.pnnl.gov/scm/stomp/stomp.git V3.0
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!----------------------Authors-----------------------------------------!
!
!     Written by MD White, PNNL, 25 February 2023.
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!
            QLVS = 0.D+0
            QCVS = 0.D+0
            DO IVS = ISRDM(8,NS),ISRDM(9,NS)
            DO JVS = ISRDM(10,NS),ISRDM(11,NS)
            DO KVS = ISRDM(12,NS),ISRDM(13,NS)
              NVS = ND(IVS,JVS,KVS)
!
!---          West surface  ---
!
