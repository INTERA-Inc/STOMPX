...
!----------------------Authors-----------------------------------------!
!
!     Written by M.D. White, PNNL, 12 January 2023.
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!
        DO NICW = ID_CW(1,NCW),ID_CW(2,NCW)
!
...
!
     ...
     &    'Well Number: ',NLW,' Leaky Well Name: ',WNM_LW(NLW)(1:NCH)
        DO NWF = ID_LW(5,NLW),ID_LW(6,NLW)
          NX = IWF_LW(NWF)
          IX = ID(NX)
          JX = JD(NX)
          KX = KD(NX)
...
!----------------------Authors-----------------------------------------!
!
!     Written by M.D. White, PNNL, 13 January 2023.
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!
        IF( ABS(VAR_CWX(2)).GT.1.D+5 ) THEN
          ID_CW(8,NCW) = 1
...
!
          ...
          IDX = 1
!
!---      Loop over the nodes in the coupled well, starting at the
!         well bottom ---
!
          DO NWN = ID_CW(4,NCW),ID_CW(3,NCW),-1
            N = IWN_CW(NWN)
...
!
        ...
        ENDDO
!
!---    Water mass balance residuals for injection type coupled well  ---
!
...
!
        DO NWN = ID_CW(4,NCW),ID_CW(3,NCW),-1
          ...
          QM_CW(3,NCW) = QM_CW(3,NCW) + Q_CW(1,NWN)
...
!
     ...
     &    ABS(QM_CW(1,NCW)).LT.ABS(VAR_CWX(2)) ) THEN
          ID_CW(8,NCW) = 0
        ENDIF
!
...
!----------------------Authors-----------------------------------------!
!
!     Written by M.D. White, PNNL, 25 January 2023.
!
!----------------------Fortran 90 Modules------------------------------!
!
      USE GLB_PAR
...
!     1 - mass injection well
!     2 - volumetric injection well
!     3 - pressure injection well
!     -1 - mass production well
!     -2 - volumetric production well
!     -3 - pressure production well
...
!     TML_CW(LN_CW) - total mass pumping limit for well, kg
!     TML_CW(LN_CW) - total volume pumping limit for well, m^3
!     VAR_CW(1,LWT_CW,LN_CW) - well time for time point for well, s
!     VAR_CW(2,LWT_CW,LN_CW) - well mass rate for time point, kg/s
!     VAR_CW(2,LWT_CW,LN_CW) - well volume rate for time point, m^3/s
!     VAR_CW(3,LWT_CW,LN_CW) - well press. limit for time point, Pa
!     VAR_CW(3,LWT_CW,LN_CW) - well pressure for time point, Pa
...
!
     ...
     &    INDEX(ADUM(1:),'injection').NE.0 ) THEN
          WRITE(IWR,'(2X,A)') 'Mass Rate Injection Well'
          WNM_CW(NCW) = 'MIW'
          ICX = ICOUNT(NCW)
          WRITE(FORM1(3:3),'(I1)') ICX
          WRITE(WNM_CW(NCW)(4:4+ICX-1),FORM1) NCW
          IT_CW(NCW) = 1
        ELSEIF( INDEX(ADUM(1:),'volu').NE.0 .AND.
     &    INDEX(ADUM(1:),'injection').NE.0 ) THEN
          WRITE(IWR,'(2X,A)') 'Volumetric Rate Injection Well'
          WNM_CW(NCW) = 'VIW'
          ICX = ICOUNT(NCW)
          WRITE(FORM1(3:3),'(I1)') ICX
          WRITE(WNM_CW(NCW)(4:4+ICX-1),FORM1) NCW
          IT_CW(NCW) = 2
        ELSEIF( INDEX(ADUM(1:),'press').NE.0 .AND.
     &    INDEX(ADUM(1:),'injection').NE.0 ) THEN
          WRITE(IWR,'(2X,A)') 'Pressure Injection Well'
          WNM_CW(NCW) = 'PIW'
          ICX = ICOUNT(NCW)
          WRITE(FORM1(3:3),'(I1)') ICX
          WRITE(WNM_CW(NCW)(4:4+ICX-1),FORM1) NCW
     ...
     &    INDEX(ADUM(1:),'mass').NE.0  ) THEN
          WRITE(IWR,'(2X,A)') 'Mass Rate Production Well'
          IT_CW(NCW) = -1
          WNM_CW(NCW) = 'MPW'
          ICX = ICOUNT(NCW)
          WRITE(FORM1(3:3),'(I1)') ICX
     ...
     &    INDEX(ADUM(1:),'volu').NE.0  ) THEN
          WRITE(IWR,'(2X,A)') 'Volumetric Rate Production Well'
          IT_CW(NCW) = -2
          WNM_CW(NCW) = 'VPW'
          ICX = ICOUNT(NCW)
          WRITE(FORM1(3:3),'(I1)') ICX
     ...
     &    INDEX(ADUM(1:),'press').NE.0  ) THEN
          WRITE(IWR,'(2X,A)') 'Pressure Production Well'
          IT_CW(NCW) = -3
          ICX = ICOUNT(NCW)
          WRITE(FORM1(3:3),'(I1)') ICX
          WNM_CW(NCW) = 'PPW'
...
!
        TML_CW(NCW) = 1.D+20
!
!---    Read total mass injected limit for mass injection wells  ---
!
...
!
!---    Read total volume injected limit for volumetric
!       injection wells or pressure injection wells  ---
!
        ELSEIF( IT_CW(NCW).EQ.2 .OR. IT_CW(NCW).EQ.3 ) THEN
          VARB = 'Total Volume Injected Limit'
          CALL RDDPR(ISTART,ICOMMA,CHDUM,TML_CW(NCW))
          CALL RDCHR(ISTART,ICOMMA,NCH,CHDUM,UNTS)
          WRITE(IWR,'(2X,4A,1PE11.4,$)') VARB(1:IVR),', ',
     &      UNTS(1:NCH),': ',TML_CW(NCW)
...
!
     ...
     &      UNTS(1:NCH),': ',TML_CW(NCW)
          ...
          WRITE(IWR,'(A,1PE11.4,A)') ' (',TML_CW(NCW),', kg)'
!
!---    Read total volumetric withdrawn/produced limit  ---
!
        ELSEIF( IT_CW(NCW).EQ.-2 .OR. IT_CW(NCW).EQ.-3 ) THEN
          VARB = 'Total Volumetric Withdrawn/Produced Limit'
          CALL RDDPR(ISTART,ICOMMA,CHDUM,TML_CW(NCW))
          TML_CW(NCW) = ABS(TML_CW(NCW))
          CALL RDCHR(ISTART,ICOMMA,NCH,CHDUM,UNTS)
          WRITE(IWR,'(2X,4A,1PE11.4,$)') VARB(1:IVR),', ',
...
!
     ...
     &        UNTS(1:NCH),': ',VAR_CW(3,M,NCW)
            ...
            WRITE(IWR,'(A,1PE11.4,A)') ' (',VAR_CW(3,M,NCW),', Pa)'
!
!---      Read injection pressure  ---
!
          ELSEIF( IT_CW(NCW).EQ.3 ) THEN
            VARB = 'Well-Top Injection Pressure'
            CALL RDDPR(ISTART,ICOMMA,CHDUM,VAR_CW(3,M,NCW))
            CALL RDCHR(ISTART,ICOMMA,NCH,CHDUM,UNTS)
            WRITE(IWR,'(4X,4A,1PE11.4,$)') VARB(1:IVR),', ',
     &        UNTS(1:NCH),': ',VAR_CW(3,M,NCW)
...
!
     ...
     &        UNTS(1:NCH),': ',VAR_CW(3,M,NCW)
            ...
            WRITE(IWR,'(A,1PE11.4,A)') ' (',VAR_CW(3,M,NCW),', Pa)'
!
!---      Read withdrawl/production volumetric flow rate  ---
!
          ELSEIF( IT_CW(NCW).EQ.-2 ) THEN
            VARB = 'Withdrawl/Production Volumetric Flow Rate'
            CALL RDDPR(ISTART,ICOMMA,CHDUM,VAR_CW(2,M,NCW))
            VAR_CW(2,M,NCW) = ABS(VAR_CW(2,M,NCW))
            CALL RDCHR(ISTART,ICOMMA,NCH,CHDUM,UNTS)
            WRITE(IWR,'(4X,4A,1PE11.4,$)') VARB(1:IVR),', ',
...
!
     ...
     &        UNTS(1:NCH),': ',VAR_CW(3,M,NCW)
            ...
            WRITE(IWR,'(A,1PE11.4,A)') ' (',VAR_CW(3,M,NCW),', Pa)'
!
!---      Read withdrawl/production pressure  ---
!
          ELSEIF( IT_CW(NCW).EQ.-3 ) THEN
            VARB = 'Well-Bottom Pressure'
            CALL RDDPR(ISTART,ICOMMA,CHDUM,VAR_CW(3,M,NCW))
            CALL RDCHR(ISTART,ICOMMA,NCH,CHDUM,UNTS)
            WRITE(IWR,'(4X,4A,1PE11.4,$)') VARB(1:IVR),', ',
     &        UNTS(1:NCH),': ',VAR_CW(3,M,NCW)
...
!
            CLX = PORD(2,N)*SL(2,N)
            IF( CLX.GT.EPSL ) SORTX = SORTX + Q_CW(1,NWN)/CLX
          ENDIF
!
!---      Load Jacobian  ---
!
